//! The elo overlay protocol, v1 — `sdk/PROTOCOL.md` is the spec.
//!
...
pub const VERBS: [(&str, &[&str]); 9] = [
    ...
    // `prove_message`. The game hands over opaque values and the launcher
    // writes the sentence. `issued_at` is opaque in the same way: a second,
    // never a string, so the launcher and not the caller decides how a
    // timestamp is spelled.
    //
    // ⚠ An older launcher refuses `issued_at` by this very table rather than
    // ignoring it, which is the right failure and a loud one: a game that
    // needs a recomputable message learns it cannot have one here, instead of
    // getting a proof stamped with a time its server cannot rebuild.
    ("prove", &["op", "nonce", "server", "issued_at"]),
    ("profile", &["op", "address"]),
    ("title", &["op", "slug"]),
    ("servers", &["op", "slug"]),
    ("open", &["op", "url"]),
...
///
/// Its own function because `elo prove` on the CLI needs exactly this check
...
/// The message `elo signin` composes to sign the WEBSITE in — the device-code
...
/// How far a caller-supplied `issued_at` may sit from this machine's clock.
///
/// Fifteen minutes each way. A server's challenge and a player's launcher are
/// two clocks that have never met, so some skew is ordinary and refusing it
/// would break honest logins on a box whose NTP has drifted. What it will not
/// do is stamp a message with a time nobody could have chosen: an `Issued At`
/// far in the past is a replay dressed as fresh, and one far in the future is a
/// signature that stays valid after the player expected it to lapse. Both are
/// the caller's to gain and the player's to lose, which is what makes this the
/// launcher's business rather than the caller's.
pub const ISSUED_AT_SKEW_SECS: i64 = 900;

/// A caller-supplied `issued_at`, in UNIX seconds, near enough to now.
pub fn check_issued_at(secs: i64) -> Result<(), Refusal> {
    let now = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .map(|d| d.as_secs() as i64)
        .unwrap_or(0);
    let drift = secs - now;
    if drift.abs() > ISSUED_AT_SKEW_SECS {
        return Err(Refusal::new(format!(
            "prove: issued_at is {drift}s from this machine's clock — more than \
             the {ISSUED_AT_SKEW_SECS}s either side a real challenge can be. \
             Check the clock on both ends before signing something dated wrong."
        )));
    }
    Ok(())
}
...
pub fn sign_family(text: &str) -> Result<String, Refusal> {
    ...
    // ⚠ `scry ` is accepted alongside `elo ` and is NOT dead code. This parses
    // a text a GAME handed us, and a game links whatever SDK version it shipped
    // with — the client renames in one commit, a released game renames when its
    // publisher cuts a build. `meter/playauth.py` verifies both for the same
    // reason and carries the same note.
    let rest = first
        .strip_prefix("elo ")
        .or_else(|| first.strip_prefix("scry "))
        .ok_or_else(|| {
            Refusal::new(
                "a signable message's first line is `elo <family>` \
