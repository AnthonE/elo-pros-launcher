...
//! `sign_siwe` calls `Elo::discover` again. A door that accepts one connection
...
//! running. `sdk/rust/elo_overlay.rs` already names it: *"a launcher that
...
use elo_broker::{transport, Host, What};
