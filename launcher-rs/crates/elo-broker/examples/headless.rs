...
//!
//! `elo-gui` is the launcher a player runs, and it links FLTK, so it cannot be
...
use elo_broker::{protocol::Refusal, serve_conn, transport::Listener, Host, What};
...
impl Headless {
    fn account() -> elo_vault::Account {
        elo_vault::Account::from_secret(&SECRET).expect("the fixed test secret is valid")
    }
}
...
impl Host for Headless {
    ...
    fn profile(&mut self, address: &str) -> Result<Value, Refusal> {
        ...
        Ok(json!({
            "reachable": true,
            "profile": elo_broker::protocol::flatten_who(&who, address),
            "source": "https://elo.test/api/who",
        }))
    }

    fn title_url(&self, slug: &str, what: What) -> Option<String> {
        match what {
            What::Manifest => Some(format!("https://elo.test/api/launcher/manifest/{slug}")),
