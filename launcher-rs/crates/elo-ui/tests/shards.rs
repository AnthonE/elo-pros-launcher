...
use fltk::prelude::*;
use elo_depot::shardlist::Shard;
use elo_ui::windows::{self, Shards};
...
fn the_window_offers_exactly_what_the_list_states() {
    ...
    assert_eq!(
        w.rows[0].link, "elo://join/gates/eu-1.example.test:61234",
        "the invite must be the canonical form"
    );
    let back = elo_depot::deeplink::parse(&w.rows[0].link).expect("the drawn link must parse");
