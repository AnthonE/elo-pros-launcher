...
use elo_depot::{install, installed, launch, parse_depot, parse_manifest, plan, verify};
use elo_net::Net;
use serde_json::json;
...
fn a_build_installs_over_real_http_verifies_and_actually_runs() {
    ...
    l.env.insert("ELO_TEST_OUT".into(), out.to_string_lossy().into());
...
fn an_update_over_http_fetches_only_what_changed() {
    ...
    let checked = elo_depot::check(&m, "linux-x86_64", tmp.path(), &net, true);
