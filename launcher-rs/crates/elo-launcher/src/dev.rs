//! `elo dev` — the desk a listing's own developer stands at.
//!
...
//!
//! **The key never leaves this machine and never reaches elo.** The signature
//! does. That is the same wall `elo account sign` stands on, and the reason a
...
//! `--json` clean: an agent reads `elo dev --json`, learns which titles it may
...
pub const USAGE: &str = "\
elo dev — a listing's own desk: the row, the art, the updates

  elo dev                          where this account stands, on every title
  elo dev show   <slug>            the row as the store serves it
  elo dev edit   <slug> --set k=v  edit your own row (repeat --set, or --from f.json)
  elo dev image  <slug> <file>     upload art; prints the url to paste into a field
  elo dev post   <slug> --title T  post a community update
  elo dev posts  <slug>            the updates already posted
  elo dev delegate <slug> <address>   let another wallet act for you
  elo dev revoke   <slug> <address>   take that back (or resign, with your own)
...
/// whole crate is built around (`elo-net`'s module note).
...
pub fn run(a: &Args, host: &str, json_out: bool) -> Result<i32, DepotError> {
    ...
    // `elo dev gates` reads as "the gates desk", so a slug in the first
    ...
    match sub {
        ...
        "whoami" => {
            ...
            match &me {
                ...
                None => println!("no account on this machine — `elo account new` makes one"),
            }
        ...
        "show" => {
            ...
            let row = cat
                ...
                .cloned()
                .ok_or_else(|| {
                    DepotError::new(format!("'{slug}' is not on the shelf — `elo games` lists it"))
                })?;
            if json_out {
        ...
        "image" => {
            ...
            println!("  wire it up: elo dev edit {slug} --set capsule={asset}");
            Ok(0)
        }
        ...
        other => Err(DepotError::new(format!(
            "unknown dev command {other:?} — `elo dev help` lists them"
        ))),
    }
}
...
mod tests {
    ...
    fn from_a_file_and_set_compose_with_set_winning() {
        let dir = std::env::temp_dir().join("elo-dev-test-fields");
