...
`meter/hive.py:verify_event` is that code.
    ...
    cargo test -p elo-hive emit_events_for_the_python_verifier
    python3 crates/elo-hive/tests/parity_verify.py

Same discipline as `elo-depot`'s digest parity, and for the same reason: a
...
if not payload_path.is_file():
    ...
    print("Run: cargo test -p elo-hive emit_events_for_the_python_verifier")
    sys.exit(2)
