...
/// NIP-01 kind:1 with `t=scry` is the town stream; a room is kind:9 `#h`.
...
mod tests {
    ...
    fn a_relay_url_splits_into_host_port_and_path() {
        assert_eq!(
            split_url("wss://hive.scry.moreright.xyz").unwrap(),
            ("hive.scry.moreright.xyz".into(), 443, "/".into())
        );
        assert_eq!(
    ...
    fn the_note_shape_matches_what_the_room_reads() {
        ...
        assert_eq!(town.tags, vec![vec!["t", "scry"]]);
