...
mod tests {
    ...
    fn the_sworn_handle_wins_over_everything() {
        ...
        let id = who(json!({"identity": {
            "nip05": "magus@scry.moreright.xyz",
    ...
    fn nobody_is_faceless() {
        ...
        assert_eq!(marked.face, "https://scry.test/vow/77c2/mark.svg");
    ...
    fn an_unsworn_address_is_bare_and_still_has_a_name_and_a_face() {
        let id = Identity::bare("0x1234567890abcdef1234567890abcdef12345678", "https://scry.test");
        assert!(id.bare);
        assert_eq!(id.kind, NameKind::Address);
    ...
    fn the_real_who_shape_resolves() {
        ...
        assert_eq!(id.face, "https://scry.test/vow/1a434221c5333fa7/mark.svg");
    ...
    fn a_bare_address_has_no_vows() {
        let id = Identity::bare("0xabc", "https://scry.test");
        assert_eq!(id.vows, 0);
        assert!(!id.ambiguous());
    }
