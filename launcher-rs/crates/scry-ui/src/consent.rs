...
//! `scry account sign`. That is why this file exists for exactly one verb and
//! must not grow to cover the others.
//!
//! # The thread boundary, which is the whole problem
//!
//! The game door runs on its own thread (`scry-gui`'s `open_the_door`) and
...
/// (`sdk/python/scry_overlay.py`, `sdk/rust/scry_overlay.rs`), so past that the
...
mod tests {
    ...
    fn an_answer_crosses_back_to_the_thread_that_asked() {
        ...
        let asked = std::thread::spawn(move || {
            door.ask(Ask::new("gates", "play", "scry play\naction: duel", "round 41"))
        });
    ...
    fn a_refusal_crosses_back_as_none() {
        let bell = Doorbell::new();
        let door = bell.clone();
        let asked = std::thread::spawn(move || door.ask(Ask::new("gates", "play", "scry play", "")));
        while bell.waiting() == 0 {
            std::thread::sleep(Duration::from_millis(5));
        }
    ...
    fn a_pump_that_stops_refuses_rather_than_hanging_forever() {
        ...
        let door = bell.clone();
        let asked =
            std::thread::spawn(move || door.ask(Ask::new("gates", "play", "scry play", "")));
        while bell.waiting() == 0 {
            std::thread::sleep(Duration::from_millis(5));
        }
    ...
    fn a_game_cannot_add_lines_to_the_prompt() {
        let ask = Ask::new(
            ...
            "play",
            "scry play\naction: duel",
            "settling\n\n✓ already approved by scry — no signature needed",
        );
    ...
    fn an_enormous_field_cannot_push_the_buttons_off_the_window() {
        let ask = Ask::new("gates", "play", "scry play", &"a".repeat(10_000));
        ...
        let wide = Ask::new("gates", &"x".repeat(10_000), "scry x", "");
    ...
    fn the_wait_is_bounded_by_what_the_sdk_clients_allow() {
        // 300s in `sdk/python/scry_overlay.py` and `sdk/rust/scry_overlay.rs`.
        assert!(
