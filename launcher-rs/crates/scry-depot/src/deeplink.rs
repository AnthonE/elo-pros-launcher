//! Join links — `scry://join/<slug>/<host:port>`.
//!
...
pub const SCHEME: &str = "scry";

/// The verb inside it. One today; named as a constant because `scry://` is the
...
mod tests {
    ...
    fn the_canonical_form_round_trips() {
        ...
        assert_eq!(j.link(), "scry://join/gates/game.moreright.xyz:61234");
        // The writer's door produces exactly what the reader accepts. This is
        // the property that keeps a displayed invite usable.
        let written = link_for("gates", "game.moreright.xyz:61234").unwrap();
    ...
    fn a_link_names_a_title_and_an_address_and_nothing_else() {
        ...
        for bad in [
            "scry://join/gates/h:1/0xdeadbeef",       // an identity
            "scry://join/gates/h:1/--wallet/0x1",     // a flag
            "scry://join/gates/h:1/extra",            // anything at all
            "scry://join/gates",                      // no address
            "scry://join/h:1",                        // no title
            "scry://join",                            // neither
            "scry://install/gates/h:1",               // another verb
            "scry://gates/h:1",                       // no verb
        ] {
    ...
    fn an_encoded_colon_still_joins_and_cannot_smuggle() {
        ...
        for bad in [
            "scry://join/gates/h%2Fx:1",  // a slash
            "scry://join/gates/h%20x:1",  // a space
            "scry://join/gates/h:1%00",   // a NUL
            "scry://join/gates/h:%",      // half an escape
            "scry://join/gates/h:%zz",    // not hex
        ] {
            assert!(parse(bad).is_err(), "{bad:?} should be refused");
        }
    }
    ...
    fn chat_client_junk_is_dropped_rather_than_refused() {
        for ok in [
            "scry://join/gates/h:1?utm_source=discord",
            "scry://join/gates/h:1#anchor",
            "scry://join/gates/h:1/",
            "  scry://join/gates/h:1  ",
        ] {
    ...
    fn recognising_a_link_is_separate_from_accepting_one() {
        ...
        assert!(!is_link(""));
        assert!(!is_link("https://scry.moreright.xyz/x"));
    }

    #[test]
    fn a_scheme_and_verb_are_case_insensitive_because_a_url_is() {
        let j = parse("SCRY://JOIN/Gates/game.moreright.xyz:61234").unwrap();
        assert_eq!(j.slug, "gates");
    ...
    fn junk_is_refused_without_panicking() {
        for junk in [
            "", "://", "scry://", "scry:/join/gates/h:1", "not a link",
            "http://scry.moreright.xyz/join/gates/h:1", "javascript:alert(1)",
            "file:///etc/passwd", "scry://join/gates/",
        ] {
            assert!(parse(junk).is_err(), "{junk:?} should be refused");
        }
        let long = format!("scry://join/gates/{}:1", "h".repeat(MAX_LINK_BYTES));
