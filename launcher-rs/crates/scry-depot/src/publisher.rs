...
//! Until this module existed, `publisher: "scryworks"` was a string somebody
...
pub const STATEMENT_VERSION: &str = "scry-manifest-v1";
...
pub fn verify(m: &Manifest) -> PublisherCheck {
    ...
    match (claimed, sig) {
        ...
        (Some(claimed), Some(sig)) => match scry_vault::recover_signer(&statement(m), sig) {
            Ok(recovered) => {
...
mod tests {
    ...
    fn the_statement_covers_every_field_it_claims_to() {
        ...
        for expect in ["slug:gates", "name:Gates", "publisher:scryworks",
