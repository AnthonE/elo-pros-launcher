...
use scry_depot::manifest::parse_manifest;
use scry_depot::path::{resolve_into, safe_relpath};
use scry_depot::{parse_depot, DEPOT_VERSION};
...
fn manifest_with_depot(depot: Value) -> scry_depot::Manifest {
    parse_manifest(&json!({
        "slug": "gates", "name": "Gates",
...
fn a_path_absolute_depot_is_completed_against_the_origin_we_asked() {
    ...
    m.rebase("https://scry.moreright.xyz");
    assert_eq!(
        m.builds[0].depot_url.as_deref(),
        Some("https://scry.moreright.xyz/api/launcher/depot/gates/0.1.0")
    );
    assert_eq!(
