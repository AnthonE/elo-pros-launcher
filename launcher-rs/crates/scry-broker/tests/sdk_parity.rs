...
//! is the contract, `sdk/rust/scry_overlay.rs` is the source a game vendors, and
...
use scry_broker::{serve_conn, transport::Listener, Host, What};
...
impl Host for RealHost {
    ...
    fn address(&self) -> Option<String> {
        scry_vault::Account::from_secret(&[9u8; 32]).ok().map(|a| a.address())
    }
    fn signer_name(&self) -> String { "arca".into() }
    fn host_url(&self) -> String { "https://scry.moreright.xyz".into() }
    ...
    fn prove(&mut self, message: &str) -> Result<Value, Refusal> {
        let account = scry_vault::Account::from_secret(&[9u8; 32])
            .map_err(Refusal::new)?;
        Ok(json!({
...
fn the_vendored_sdk_proves_an_identity_a_server_can_verify() {
    with_broker(true, |endpoint| {
        ...
        let issued_at = proof.message
            ...
            .to_string();
        let account = scry_vault::Account::from_secret(&[9u8; 32]).expect("account");
        let expected = scry_broker::protocol::prove_message(
        ...
        let recovered = scry_vault::recover_signer(&proof.message, &proof.signature)
        ...
        let other = scry_broker::protocol::prove_message(
            "shard-3.gates.example", &account.address(), "gates",
            "aDifferentNonce1", &issued_at);
        let elsewhere = scry_vault::recover_signer(&other, &proof.signature)
        ...
        let other_game = scry_broker::protocol::prove_message(
            "shard-3.gates.example", &account.address(), "some-other-title",
            "8f14e45fceea167a", &issued_at);
        let cross = scry_vault::recover_signer(&other_game, &proof.signature)
            .expect("recovery still yields some address");
        ...
        let other_domain = scry_broker::protocol::prove_message(
            "evil.example", &account.address(), "gates",
            "8f14e45fceea167a", &issued_at);
        let elsewhere2 = scry_vault::recover_signer(&other_domain, &proof.signature)
            .expect("recovery still yields some address");
