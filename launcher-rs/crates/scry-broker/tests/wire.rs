...
use scry_broker::{serve_conn, Host, What};
...
fn the_vocabulary_cannot_operate_the_launcher() {
    let caps = scry_broker::capabilities();
...
fn a_signable_message_must_declare_its_family_on_the_first_line() {
    ...
    assert!(out[1]["reason"].as_str().unwrap().contains("scry <family>"));
}
...
fn consent_is_counted_and_a_second_ask_reprompts() {
    ...
    let sign = json!({"op": "sign", "text": "scry play\nround 41", "why": "settling"});
...
/// (`scry_ui::consent::one_line`); a broker that sanitised on the way through
...
fn a_sign_with_no_why_still_asks() {
    let mut h = Fake::default();
    let out = talk(&mut h, &[hello(), json!({"op": "sign", "text": "scry play\nx"})]);
    assert_eq!(out[1]["ok"], json!(true));
...
impl scry_broker::Host for Prover {
    ...
    fn host_url(&self) -> String { "https://scry.example".into() }
    fn ask_consent(&mut self, _g: &str, _f: &str, _t: &str, _w: &str) -> Option<u32> {
        *self.consents.borrow_mut() += 1;
        Some(1)
    }
    fn sign(&mut self, _t: &str, _w: &str) -> Result<serde_json::Value, scry_broker::protocol::Refusal> {
        Err(scry_broker::protocol::Refusal::new("no"))
    }
    fn prove(&mut self, message: &str) -> Result<serde_json::Value, scry_broker::protocol::Refusal> {
...
fn prove_call(host: &mut Prover, nonce: &str, server: &str) -> serde_json::Value {
    let mut s = scry_broker::Session::default();
...
fn prove_signs_a_message_the_launcher_wrote() {
    ...
    assert_eq!(
        signed,
        scry_broker::protocol::prove_message(
...
fn a_proof_signed_by_the_wrong_account_is_refused() {
    struct Mismatched;
    impl scry_broker::Host for Mismatched {
        fn address(&self) -> Option<String> {
            Some("0x24445EFddB08d4938E3E3627042B2Cf4063d9092".into())
        }
        fn signer_name(&self) -> String { "local".into() }
...
fn a_host_that_did_not_implement_prove_refuses() {
    struct Bare;
    impl scry_broker::Host for Bare {
