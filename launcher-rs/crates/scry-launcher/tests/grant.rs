...
//!
//! `SCRY_GRANT` is a bearer token for one game's downloads. The one thing
...
fn grant_goes_to_the_origin_and_nowhere_else() {
    ...
    std::env::set_var("SCRY_GRANT", "gates.0xabc.9999999999.deadbeef");
    std::env::set_var("SCRY_GRANT_ORIGIN", &origin);
    ...
    assert!(
        head.to_lowercase().contains("x-scry-grant: gates.0xabc.9999999999.deadbeef"),
    ...
    let head = h.join().expect("join");
    assert!(
        !head.to_lowercase().contains("x-scry-grant"),
        "the grant is a bearer token and must NOT be mailed to a foreign host; head:\n{head}"
    );

    std::env::remove_var("SCRY_GRANT");
    std::env::remove_var("SCRY_GRANT_ORIGIN");
}
