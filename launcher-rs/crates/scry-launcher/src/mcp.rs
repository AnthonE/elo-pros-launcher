//! `scry mcp` — the launcher's machine face, on stdio.
//!
...
fn tool_schema() -> Value {
    ...
    json!({
        "name": "launcher",
        "description": format!(
            "Read the scry desktop client on THIS machine: {described}. \
...
impl Server {
            ...
            "initialize" => {
                ...
                ok_reply(
                    ...
                    json!({
                        ...
                        "instructions": "The scry desktop client on this machine. \
    ...
    fn library(&self) -> String {
        let rows = scry_depot::installed(&self.games);
        if rows.is_empty() {
            return format!(
    ...
    fn sessions(&self) -> String {
        ...
        for s in all.iter().filter(|s| s.is_running()) {
            if let scry_depot::Ending::Running { pid } = s.ending {
    ...
    fn status(&self, slug: &str) -> Result<String, String> {
        let installs: Vec<_> = scry_depot::installed(&self.games)
            .into_iter()
        ...
        out.push_str(
            "\nWhether a NEWER build is published needs the title's manifest url, \
             which this server does not store. Run `scry status <manifest>` — it \
    ...
    fn talk(&self, room: Option<&str>, limit: usize) -> Result<String, String> {
        ...
        let mut out = format!("{}\n", r.scope);
        for m in &rows {
            // The same resolver the terminal and `scry who` use.
...
/// `scry mcp` — serve until stdin closes.
