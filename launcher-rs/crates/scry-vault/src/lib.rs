...
//!
//! `WALLETS.md` §0 carries *"Scry never creates an account, holds a seed, or
...
//! no user keys, and scry the service never sees this one. It is generated from
...
///
/// Both binaries ask here. That matters more than it looks: `scry account new`
...
pub fn keystore_path() -> std::path::PathBuf {
    if let Ok(p) = std::env::var("SCRY_KEYSTORE") {
        return std::path::PathBuf::from(p);
    }
    #[cfg(windows)]
    {
        let base = std::env::var("APPDATA").unwrap_or_else(|_| ".".into());
        std::path::PathBuf::from(base).join("scry").join("keystore.json")
    }
    #[cfg(not(windows))]
    {
