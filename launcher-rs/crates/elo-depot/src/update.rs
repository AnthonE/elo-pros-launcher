...
/// Every installed build of one title, in the order `installed` returns them
/// — which is **name order, and name order is not install order.** For the
/// one you should run, see [`newest_of`].
...
/// When a build was installed: the mtime of its directory, which `install`
/// writes and nothing else touches. `UNIX_EPOCH` for a directory we cannot
/// stat, so an unreadable one sorts oldest rather than winning by accident.
fn installed_at(p: &Path) -> std::time::SystemTime {
    std::fs::metadata(p)
        .and_then(|m| m.modified())
        .unwrap_or(std::time::UNIX_EPOCH)
}

/// The build of a title that was installed **most recently** — the one to run,
/// to keep, and to compare against the origin.
///
/// ⚠ **Not the last one by name, and that mistake shipped.** A build id is
/// `<version>-g<short sha>`; a sha is hex and carries no order at all, so
/// sorting ids alphabetically shuffles them. Three call sites took `rfind` over
/// the sorted list and called it the newest, with a comment explaining why the
/// FIRST match would be wrong — the reasoning was sound and the premise was
/// false. On 2026-08-15 a player with six builds of one title clicked a library
/// row that correctly read *up to date* and launched a build from four days
/// earlier: `0.2.0-gfebc701e4` sorts after `0.2.0-g22574a33b` because `f` beats
/// `2`. They got a client missing the fix they had just downloaded, and every
/// surface said they were current.
///
/// Recency is a property of the filesystem, not of a name.
pub fn newest_of(games_root: &Path, slug: &str) -> Option<Install> {
    builds_of(games_root, slug)
        .into_iter()
        .max_by_key(|i| installed_at(&i.path))
}
...
/// nudge. A disagreement between them costs one wrong sentence in `elo check`
