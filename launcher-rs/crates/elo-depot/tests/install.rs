...
use elo_depot::install::{human, Plan};
use elo_depot::update::{JsonSource, UpdateState};
use elo_depot::{
...
fn manifest_pointing_at(depot_url: &str) -> elo_depot::Manifest {
    parse_manifest(&json!({
        "slug": "t", "name": "T",
...
fn the_updater_reports_current_stale_and_not_installed() {
    ...
    let c = elo_depot::check(&m, "linux-x86_64", tmp.path(), &Source(Ok(d1.raw.clone())), false);
    ...
    let d2 = depot_for("2", &v2);
    let c = elo_depot::check(&m, "linux-x86_64", tmp.path(), &Source(Ok(d2.raw.clone())), false);
    match &c.state {
    ...
    let c = elo_depot::check(&m, "linux-x86_64", tmp.path(), &Source(Ok(repack.raw)), false);
...
fn a_slot_reads_as_unpublished_not_as_an_update() {
    ...
    .unwrap();
    let c = elo_depot::check(&m, "linux-x86_64", tmp.path(), &Source(Ok(json!({}))), false);
...
fn superseded_lists_the_old_builds_and_never_the_kept_one() {
    ...
    let old = elo_depot::superseded(tmp.path(), "t", "2");
    assert_eq!(old.len(), 1);
...
fn a_game_can_ask_where_its_shard_list_is() {
    ...
    let vals = BTreeMap::from([(
        "servers".to_string(),
        "https://elo.example/api/launcher/servers/gates".to_string(),
    )]);
    let l = launch::resolve(&i, &vals, &BTreeMap::new()).unwrap();
    assert_eq!(
        &l.argv[1..],
        &["--servers", "https://elo.example/api/launcher/servers/gates"]
    );
    ...
    // question, and `elo-gui.rs`'s own source scan is what holds the call
    // sites to it — a rule with no owner is a rule each new door re-derives.
    assert_eq!(
        launch::title_values(Some("https://elo.example/api/launcher/servers/gates")),
        BTreeMap::from([(
            "servers".to_string(),
            "https://elo.example/api/launcher/servers/gates".to_string()
        )]),
...
fn the_launcher_env_wins_over_the_depots() {
    ...
    install(&depot_for("1", &files), tmp.path(), &serving(served, None), None, None).unwrap();
    let mut i = installed(tmp.path()).remove(0);
    i.launch_env = BTreeMap::from([("ELO_LAUNCHER_SOCKET".into(), "/tmp/evil.sock".into())]);
    let ours = BTreeMap::from([("ELO_LAUNCHER_SOCKET".to_string(), "/run/real.sock".to_string())]);
    let l = launch::resolve(&i, &BTreeMap::new(), &ours).unwrap();
    assert_eq!(l.env["ELO_LAUNCHER_SOCKET"], "/run/real.sock");
}
...
fn a_browser_row_opens_only_http_and_supports_app_mode() {
    for bad in ["file:///etc/passwd", "steam://run/440", "javascript:alert(1)", "play"] {
        assert!(elo_depot::browser_argv(bad, "").is_err(), "should refuse {bad}");
    }
    assert_eq!(
        elo_depot::browser_argv("https://x/play", "").unwrap(),
        ["xdg-open", "https://x/play"]
    );
    // `{url}` is what app mode needs — `--app=URL` must be ONE argument, which
    // appending the url never produces.
    assert_eq!(
        elo_depot::browser_argv("https://x/play", "chromium --app={url}").unwrap(),
        ["chromium", "--app=https://x/play"]
    );
    assert_eq!(
        elo_depot::browser_argv("https://x/play", "firefox --kiosk").unwrap(),
