...
use elo_vault::{eip191_digest, keystore, to_checksum, Account};
...
fn a_signature_is_65_bytes_of_hex_with_an_ethereum_v() {
    let acct = Account::generate().expect("entropy");
    let sig = acct.sign_message("elo play\nround 41").expect("sign");
    ...
    assert!(elo_broker_shape(&sig), "the external signer's own check must accept it");
}

/// The same predicate `ExternalSigner` applies, restated here so the two halves
/// cannot drift apart silently.
fn elo_broker_shape(s: &str) -> bool {
    let s = s.trim();
...
fn a_locked_account_shows_the_same_address_as_an_unlocked_one() {
    ...
    let created = match elo_vault::LocalSigner::create(&path, "pass") {
        Ok(s) => s,
        Err(e) => panic!("create: {e}"),
    };
    let unlocked_addr = created.address().expect("an unlocked account knows itself");

    let mut reopened = elo_vault::LocalSigner::at(&path);
...
fn a_locked_account_refuses_to_sign() {
    ...
    let dir = tempfile::tempdir().expect("tmp");
    let path = dir.path().join("keystore.json");
    let _ = elo_vault::LocalSigner::create(&path, "pass").map_err(|e| panic!("create: {e}"));

    let mut locked = elo_vault::LocalSigner::at(&path);
    let e = locked.sign("elo play\nx", "y").expect_err("locked must not sign");
...
fn creating_over_an_existing_keystore_is_refused() {
    let dir = tempfile::tempdir().expect("tmp");
    let path = dir.path().join("keystore.json");
    let _ = elo_vault::LocalSigner::create(&path, "pass").map_err(|e| panic!("first: {e}"));
    let e = match elo_vault::LocalSigner::create(&path, "other") {
...
fn the_document_has_every_field_a_foreign_reader_needs() {
    let doc = match elo_vault::encrypt(&[7u8; 32], "pass", "0x0000000000000000000000000000000000000000") {
        Ok(d) => d,
    ...
    let other = elo_vault::encrypt(&[7u8; 32], "pass", "0x0000000000000000000000000000000000000000")
        .expect("second");
...
fn importing_a_known_key_keeps_its_address() {
    ...
    let imported = match elo_vault::LocalSigner::import(&path, "pass", &secret) {
        Ok(s) => s,
...
fn importing_over_an_existing_keystore_is_refused() {
    ...
    let path = dir.path().join("keystore.json");
    let _ = elo_vault::LocalSigner::create(&path, "pass").map_err(|e| panic!("first: {e}"));
    let e = match elo_vault::LocalSigner::import(&path, "other", &[9u8; 32]) {
...
fn a_signature_recovers_to_the_account_that_made_it() {
    ...
    let mut secret = [0u8; 32];
    secret[31] = 1;
    let signer = match elo_vault::LocalSigner::import(&path, "pass", &secret) {
        Ok(s) => s,
        Err(e) => panic!("import: {e}"),
    };
    ...
    let message = "elo manifest\ngates\n0.1.0";
    let sig = account.sign_message(message).expect("sign");

    assert_eq!(elo_vault::recover_signer(message, &sig).as_deref(), Ok(address.as_str()),
    ...
    let tampered = elo_vault::recover_signer("elo manifest\ngates\n0.2.0", &sig)
...
fn the_recovery_byte_accepts_wallets_and_refuses_eip155() {
    ...
    assert_eq!(elo_vault::recover_signer("hello", &sig).as_deref(), Ok(address.as_str()));
    ...
    assert_eq!(elo_vault::recover_signer("hello", &raw_v).as_deref(), Ok(address.as_str()));
    ...
    assert!(elo_vault::recover_signer("hello", &tx_v).is_err(),
            "an EIP-155 recovery byte must be refused");

    // And a truncated signature says the shape rather than panicking.
    assert!(elo_vault::recover_signer("hello", "0xdeadbeef").is_err());
}
