...
| 2 | notarize the depot digests | ⛔ **DONE FOR A BUILD NOBODY SHIPS — RE-OPENED 2026-08-21.** The seal is a fact about a **digest**, so it is **per build, every build**, and this row read a flat *done* for eleven days. What was sealed on 2026-08-10 is `0.1.0`: `linux-x86_64` `0x50fef181…8fbb`, `win-x86_64` `0xdf98de59…45cc`, on `ScryNotary` `0x0C15fA78…` under label `gates-build`. What `published.json` actually serves is **`0.5.0-gce57499e6`**, whose digests are `0xc1ec6425…6b8f` (linux) and `0x15f8b065…9d38` (win) — and both read **`firstSeen == 0`**, measured 2026-08-21. So the published bytes carry no seal at all. ⚠ **Two things hid it**: `launch_preflight.py` joined `<build>/depot.json` instead of `<build>/<platform>/depot.json`, so no digest was computable and the notary check never ran; and when it did run, urllib's default UA drew a bare 403 from the RPC, which the gate honestly reported as *the read failed* rather than as *no seal*. Both fixed 2026-08-21. **Derive it, never read this row**: `python3 deploy/launch_preflight.py gates --rpc <rpc>`. `elo digest <depot>` recomputes either digest | **owed — one `notarize` per platform** |
| 3 | `elo-shardlist-v1` served by Gates | **DONE — measured 2026-08-21.** `$SCRY_DEPOTS_DIR/gates/servers.json` is on the box and `manifest.servers.url` names `/api/launcher/servers/gates`, which serves it byte-for-byte; the one row is `game.elopros.com:61234` and it carries a live `status_url`. This row read *unbuilt* while it was serving. ⚠ The manifest emitted `kind: scry-shardlist-v1` until 2026-08-21; the client never checks the kind (it requires an object with `servers`), so nothing was broken — but a writer emitting the retired form is the half of the rename that does not fix itself | no |
