...
import {ScryGameTicketFactory} from "./ScryGameTicketFactory.sol";

interface IKnownCode {
    function deployedByFactory(address) external view returns (bool);
}

/// @title ScryGameDirectory — the store's shelf, as one address a stranger can read
...
///         is showing. No API key, no cooperation from scry, no trust in this
...
contract ScryGameDirectory {
    ...
    // ── the shelf ────────────────────────────────────────────────────────────
    struct Listing {
        address ticket; // the copy — ScryGameTicket. Prices live THERE, never here
    ...
    function listNew(
        ...
        ScryGameTicketFactory.TicketParams calldata params,
        bytes32 salt,
        address factory,
        address coin,
        address steward,
        ...
        ticket = ScryGameTicketFactory(factory).deploy(creationCode, params, salt);
