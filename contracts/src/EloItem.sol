...
/// @title EloItem — one catalog's items, and not one trait among them
...
///         requirement rather than a style. `EloGacha` is DEPLOYED, moves
...
///         ceiling `EloEidolon` carries. A creator who wants more deploys
///         something else and wears the "unknown code" badge (§1b).
contract EloItem is ReentrancyGuard {
    ...
    /// Minted by consuming other items — `EloKiln` firing a card set into a
    ...
    /// (`EloHarvest`'s stated pattern, same merkle dialect).
    ///
    ...
    /// Sorted-pair hashing, identical to `EloVowRegistry.verifyProof` — the one
