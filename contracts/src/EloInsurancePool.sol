...
/// @title EloInsurancePool — a premium pool for the trust menu
...
contract EloInsurancePool is ReentrancyGuard {
    ...
    constructor(IERC20 _reserve, uint256 _maxPayout) {
        reserve = _reserve;
        operator = msg.sender;
    ...
    function payPremium(uint256 amount) external {
        ...
        SafeERC20.safeTransferFrom(address(reserve), msg.sender, address(this), amount, "pull failed");
    ...
    function claim(address to, uint256 amount, uint256 jobId) external nonReentrant returns (uint256 paid) {
        ...
        if (paid > 0) {
            ...
            SafeERC20.safeTransfer(address(reserve), to, paid, "payout failed");
