...
///         `ScryVowRegistry` and refuses a vowId nobody ever swore. The gate is
...
contract ScrySteleEdition is ReentrancyGuard {
    ...
    IScryVowRegistryView public immutable registry;
    ...
    constructor(
        ...
        IScryVowRegistryView _registry
    ) {
    ...
    function mintEdition(bytes32 vowId) external returns (uint256 tokenId) {
        ...
        SafeERC20.safeTransferFrom(address(scry), msg.sender, feeSplitter, price, "scry payment failed");
    ...
    function contractURI() external view returns (string memory) {
        ...
        string memory json = string.concat(
            '{"name":"scry stele editions",'
            '"description":"Transferable prints of scry vow steles. The vow itself is soulbound forever '
            "at ScryVowRegistry and is NEVER transferred by a print. Minting a print says nothing about a "
            "vow's trajectory - that is the meter's signed read, never a token.\",",
