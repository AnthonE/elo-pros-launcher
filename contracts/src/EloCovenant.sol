...
/// @title EloCovenant — a fleet swears one oath, in the open, one wallet at a time
...
///         discipline: measurement is what reserve sells; the register is
...
contract EloCovenant {
    ...
    /// covenantId (content-addressed by the reserve API) => covenant
    ...
    event Sworn(bytes32 indexed covenantId, address indexed member, uint32 seq, uint64 at);
    event Renounced(bytes32 indexed covenantId, address indexed member, uint32 seq, string reason, uint64 at);
    ...
    /// @param covenantId content-addressed id from the reserve API (bytes32)
    ...
    function swear(bytes32 covenantId) external returns (uint32 seq) {
        ...
        seq = ++c.memberCount;
        c.activeCount += 1;
        membership[covenantId][msg.sender] = Membership({seq: seq, swornAt: uint64(block.timestamp), renouncedAt: 0});
