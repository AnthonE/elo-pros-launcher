...
/// @title EloNotary — commit anything, in the open, readable by humans
/// @notice The on-chain service version of what reserve already does for
...
///         discipline: measurement is what reserve sells; notarization is
...
///         a verdict.
contract EloNotary {
    string public constant NOTICE = "public commitment registry - records that a claim was made, never that it is true";
    ...
    event Notarized(
        address indexed by, bytes32 indexed hash, string label, string memo, uint64 at, uint256 seq, bool firstForHash
    );
    ...
    function notarize(bytes32 hash, string calldata label, string calldata memo) external returns (uint256 seq) {
