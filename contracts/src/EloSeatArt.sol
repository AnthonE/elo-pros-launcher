...
/// @title EloSeatArt — the Elo Pros's face, drawn on chain, owned by nobody
...
///
///         ⚠ THE JUSTIFICATION FOR THAT CHANGED 2026-08-19, THE DESIGN DID NOT.
///         This block used to add that no-setters was "a strictly stronger
///         answer to the question this audience actually asks, and it costs
///         nothing" — the rug-screen frame the operator struck, and the second
///         half was wrong: `collectionName`, `collectionDescription` and
///         `artBase` are welded, the platform has renamed to Elo Pros, and the
///         reserve ticker is moving SCRY → ELO, so all three are odds-on to
///         need correcting.
///
///         **The real reason there are no setters is the size budget, and it is
///         measured.** Adding an owner plus a `setText`/`sealText` pair was
///         BUILT and REVERTED the same day: it compiled at **24,492 bytes
///         against EIP-170's 24,576**, leaving 84 bytes of headroom where this
///         contract has 1,598. A renderer that cannot survive its next edit is
///         a worse trap than the one the setters were removing.
///
///         **And the escape hatch already exists and costs almost nothing.**
///         This contract holds no user state, so correcting a name is a fresh
///         deploy plus `EloSeat.setRenderer` — no token stranded, nothing
///         migrated. `EloSeat.renderer` stays settable until
///         `freezeMetadata()`, so **do not run that freeze until the collection
///         name and the reserve ticker have stopped moving.** That ordering is
///         the whole fix and it needs no bytecode.
///
///         ⚠ Before re-proposing setters here: run `--bin-runtime` first. The
///         numbers above are why this looks like an oversight and is not.
///
...
///
///         The reveal rides `EloSeat`'s commitment: `sha256(salt)` is posted
...
contract EloSeatArt {
    ...
    // ── tokenURI ─────────────────────────────────────────────────────────────
    /// @param salt `EloSeat.revealedSalt()` — empty until the run closes.
    ...
    ///
    /// A gold Nu on near-black, headset on, green-eyed, with a palm tuft, haloed
    /// and a ward on the belly. It was a BLUE Nu with gold eyes and no hardware
    /// until 2026-08-20, when it became the platform's logo as well as the
    /// collection's mark and the operator picked the gold.
    ///
    /// ⚠ THE IRIS IS THE PART THAT LOOKS ARBITRARY AND IS NOT. Gold eyes on a
    /// gold hide is the exact collision `_socket()` exists for, and on this
    /// vector the derivation is not enough to save it: rendered at a favicon's
    /// 16px the eyes disappear into the face and the mark reads as a featureless
    /// gold blob. Malachite is what puts them back, and it doubles as the site's
    /// own green. Anything but `gold` and `brass` clears it — those two are the
    /// ones to never write here. Measured, not guessed: render it and look.
    function emblemTraits() public pure returns (uint8[N_LAYERS] memory t) {
        t[L_SPREAD] = 2; // ink — near-black, so the gold and the green carry
        t[L_FRAME] = 0; // none
        t[L_HIDE] = 4; // gild — the collection's own gold
        t[L_BODY] = 0; // round
        t[L_EYE] = 0; // wide
        t[L_GEAR] = 2; // headset — these are Elo PROS
        t[L_MARK] = 3; // ward
        t[L_AURA] = 4; // halo
        t[L_IRIS] = 3; // malachite — NOT gold: see the warning above
    ...
    function contractURI() external view returns (string memory) {
        ...
        bytes memory json = abi.encodePacked(
            ...
            '","description":"',
            collectionDescription,
            '","external_link":"https://elopros.com/mint.html"',
            ',"image":"data:image/svg+xml;base64,',
