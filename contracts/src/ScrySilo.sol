...
import {ScryGranary} from "./ScryGranary.sol";
import {SafeERC20} from "./SafeERC20.sol";
import {ReentrancyGuard} from "./ReentrancyGuard.sol";
import {Math} from "./Math.sol";

/// @title ScrySilo - sealed spoils lockers (the xJEWEL/cJEWEL homage, FARMING.md)
...
///           routed penalties to project pots; scry retires them -
...
///
///         scry's structural corrections, same as the Gardener:
///         - the reward is a game coin - never SCRY (the money token
///           does not inflate to pay for parking);
///         - every mint goes through the ScryGranary behind a posted daily
...
///           of compounding fine print scry does not do;
...
///         not promised APY.
contract ScrySilo is ReentrancyGuard {
    // -- accounting ---------------------------------------------------------
    struct Tier {
    ...
    ScryGranary public immutable granary; // reward mints go through here
    uint256 public immutable maxBreakBps; // seal-break burn at t=0; <= 5000
    address public owner;

    /// @notice The same brick ScryGardener.MAX_REWARD_PER_SECOND removes on
    ...
    constructor(ScryGranary _granary, uint256 _rewardPerSecond, uint256 _maxBreakBps) {
    ...
    /// Ported verbatim from ScryGardener.carryScaled (operator decision
    ...
    function updateBin(uint256 binId) public {
        ...
        uint256 elapsed = block.timestamp - b.lastRewardTime;
        // mulDiv, for the same reason as ScryGardener.updatePool: bounded
    ...
    function seal(uint256 binId, uint256 tierId, uint256 amount) external nonReentrant returns (uint256 sealId) {
        ...
        // the same one ScryOrchard states: `b.spoils` is TYPED `SpoilsToken`,
        ...
        // a staticcall. ScryBank and ScryGardener DID need `pullExact` because
    ...
    function addBin(SpoilsToken spoils, uint256 allocPoint) external onlyOwner {
        ...
        // THE SAME ONE-LINE GUARD ScryGardener.addPool GOT ON 2026-07-28, and
