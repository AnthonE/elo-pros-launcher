...
/// @title EloOrchard - the v3-staker farm for the canonical pools (FARMING.md)
...
///         using MasterChef math. The Orchard is reserve's rewrite of the
...
///         The pool's price is still never an oracle for any reserve system -
...
contract EloOrchard is ReentrancyGuard {
    struct Incentive {
    ...
    constructor(SpoilsToken _rewardToken, INonfungiblePositionManagerOrchard _npm, IUniswapV3FactoryOrchard _factory) {
        // Third of the same shape, after EloGardener.addPool (07-28) and
        // EloSilo.addBin (07-29). `SpoilsToken` here is a cast over a raw
