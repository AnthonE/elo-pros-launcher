...
/// @title ScryGameTicketFactory — one place that mints a title's ticket, so its code is known
/// @notice PERMISSIONLESS ON PURPOSE, and that is not a loose end. Anyone can
///         deploy a `ScryGameTicket` with `forge create` today; a factory that
...
///         `ScryGameDirectory`, which is keeper-gated. A game deployed here and
...
///         `new ScryGameTicket(...)` — embeds the ticket's whole creation code
...
///         broadcast. Same pattern `ScryPeculiumFactory` already uses here.
///
...
contract ScryGameTicketFactory {
    string public constant NOTICE = "deploys a game's ticket contract from one pinned build - permissionless, free, "
        "and NOT a listing: deployedByFactory is a known-code signal only. Curation lives in ScryGameDirectory";

    /// keccak256 of the exact `ScryGameTicket` creation code this factory will
    ...
    struct TicketParams {
        ...
        address scry;
        address usdg;
        address scrySink; // the deployed ScryFeeSplitter — where the SCRY leg burns
