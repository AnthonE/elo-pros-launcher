...
import {IEloArbiter} from "./IEloArbiter.sol";
...
/// @title EloJobBoard — hire a named worker for a task, with a trust mode
...
///         bounded by the panel design in EloArbiter, not by this contract.
contract EloJobBoard is ReentrancyGuard {
    enum Mode {
        Escrow,
        Insured,
        ReputationOnly
    ...
    address public feeSplitter; // EloFeeSplitter — every sale's cut lands here
    IEloArbiter public arbiter;
    ...
    event Ruled(uint256 indexed id, IEloArbiter.Verdict verdict);
        ...
        returns (uint256 id)
    {
        ...
        if (mode == Mode.Escrow) {
            SafeERC20.safeTransferFrom(address(reserve), msg.sender, address(this), amount, "escrow pull failed");
            ...
            SafeERC20.safeTransferFrom(address(reserve), msg.sender, address(insurancePool), premium, "premium pull failed");
        }
        id = jobs.length;
    ...
    function dispute(uint256 id) external nonReentrant {
        ...
        if (fee > 0) SafeERC20.safeTransferFrom(address(reserve), msg.sender, arbiter.feeRecipient(), fee, "arb fee failed");

        // Reverts if the panel has not ruled — the whole dispute rolls back,
        // fee and status change with it.
        IEloArbiter.Verdict v = arbiter.rule(id, j.specHash, j.deliveredHash);
        emit Ruled(id, v);

        if (v == IEloArbiter.Verdict.ForSeller) {
            _payoutSeller(j, id, true); // deliverable met the committed spec
        } else if (v == IEloArbiter.Verdict.ForBuyer) {
    ...
    function _payoutSeller(Job storage j, uint256 id, bool coverIfUnfunded) internal {
        ...
        if (j.mode == Mode.Escrow) {
            SafeERC20.safeTransfer(address(reserve), j.seller, net, "pay seller failed");
            if (fee > 0) SafeERC20.safeTransfer(address(reserve), feeSplitter, fee, "fee failed");
        } else {
            ...
            if (pulled) {
                ...
                require(coverIfUnfunded, "buyer has not funded - approve the reserve or await deadline");
                paid = 0;
                feePaid = 0;
    ...
    function _refundBuyer(Job storage j, uint256 id, bytes32 reason) internal {
        if (j.mode == Mode.Escrow) {
            SafeERC20.safeTransfer(address(reserve), j.buyer, j.amount, "refund failed");
        }
    ...
    function _tryPull(address from, address to, uint256 amount) internal returns (bool) {
        (bool ok, bytes memory ret) =
            address(reserve).call(abi.encodeWithSelector(IERC20.transferFrom.selector, from, to, amount));
    ...
    function setArbiter(IEloArbiter _arbiter) external onlyOperator {
        arbiter = _arbiter;
    ...
    /// @notice Rotate the operator. Same shape as EloReputation /
    /// EloInsurancePool / EloHarvest / EloFeeSplitter: the admin can be
