...
/// @title EloCistern — the drop bar: fee revenue in, a public round out
...
///
///         **And it holds no key to THE POT.** No pause, no upgrade path, no
///         admin withdrawal of `reserve` — the operator's power over the bar is
///         proposing knobs that take three days to land. Pot value leaves by
///         `open` → `claim`, or it recycles into the next round.
///
///         ⚠ **`rescue()` EXISTS, AND IT CANNOT REACH THE POT.** This block
///         used to say there was no rescue at all, justified as *"the sentence
///         a rug screen is actually reading: can the team take my money"* —
///         reputational reasoning, struck 2026-08-19. What it actually bought
///         was a permanent trap: the documented funding path is `collect()` a
///         v3 position → swap the WETH leg → transfer SCRY in, and one mispaste
///         stranded the WETH here for the life of the chain.
///
///         The untouchable-pot property is **mechanical, not a promise**:
///         `pooled()` is a bare `reserve.balanceOf(this)` minus what open rounds
///         owe, and `rescue()` reverts on `reserve`. So the pot is exactly as
///         unreachable as it was, and a foreign token is no longer destroyed by
///         a typo.
///
...
///         `SENTENCES.md`, and the same thing `EloBank` says about itself.
contract EloCistern is ReentrancyGuard {
    ...
    IEloSeat public immutable seat;
    ...
    event Claimed(uint256 indexed roundId, uint256 indexed tokenId, address indexed to, uint256 reserveShare);
    ...
    constructor(
        ...
        require(address(_reserve) != address(0), "zero elo");
    ...
    function fill(uint256 amount) external nonReentrant {
        ...
        SafeERC20.safeTransferFrom(address(reserve), msg.sender, address(this), amount, "fill failed");
    ...
    function open() external nonReentrant returns (uint256 roundId) {
        ...
        if (tip > 0) SafeERC20.safeTransfer(address(reserve), msg.sender, tip, "tip failed");
        ...
        returns (uint256 reserveShare)
    {
        ...
        reserveShare = r.pot * w / r.totalWeight;
        require(reserveShare > 0, "share rounds to nothing");

        claimed[roundId][tokenId] = true;
        r.paid += reserveShare;
        outstanding -= reserveShare;
        emit Claimed(roundId, tokenId, holder, reserveShare);

        _payOut(roundId, tokenId, holder, reserveShare, minOut);
    }
        ...
        internal
    {
        ...
        if (sum != 10_000) {
            ...
            SafeERC20.safeTransfer(address(reserve), holder, share, "elo payout failed");
            emit Paid(roundId, tokenId, address(reserve), share, share);
            return;
        }
        ...
        internal
    {
        ...
        if (trackId == 0 || coin == address(0) || fee == 0 || address(router) == address(0)) {
            SafeERC20.safeTransfer(address(reserve), holder, amt, "elo leg failed");
            emit Paid(roundId, tokenId, address(reserve), amt, amt);
            return;
        }
        SafeERC20.safeApprove(address(reserve), address(router), amt, "approve failed");
    ...
    function proposeRoute(uint16 trackId, address coin, uint24 fee) external onlyOperator {
        require(trackId != 0, "track 0 is the reserve");
        require(coin != address(reserve), "track 0 is the reserve");
    ...
    /// Recover a token that is NOT the pot.
    ///
    /// ⚠ `reserve` is refused, and that refusal is the whole safety argument —
    /// it is not a policy this contract could be talked out of. `pooled()` is
    /// `reserve.balanceOf(this) - outstanding`, so with `reserve` unreachable here
    /// there is no path by which the operator touches a round's pot or the
    /// bar's level. Everything else that lands at this address — a WETH leg
    /// pasted to the wrong recipient, a game coin someone airdropped, a
    /// stray NFT's ERC-20 sibling — was previously destroyed on arrival.
    ///
    /// Deliberately NOT permissionless: a caller-aimed `to` on an open rescue
    /// is a griefing hole, and unlike the crank there is nothing here a
    /// stranger needs to be able to do.
    function rescue(address token, address to, uint256 amount) external onlyOperator {
        require(token != address(reserve), "the pot is not rescuable");
        require(to != address(0), "zero recipient");
        SafeERC20.safeTransfer(token, to, amount, "rescue failed");
