...
///         `ScryFeeSplitter` routing to `SCRY_BURN`, its own `ScryBank`, and the
...
contract ScryFeeConverter is ReentrancyGuard {
    string public constant NOTICE = "converts a launch position's fee income into SCRY before paying it out, so a "
        "studio never holds a bag of its own game's coin - the coin leaves only as a swap, SCRY leaves only to the "
    ...
    uint256 public totalPaid; // scry out

    event Converted(uint256 coinIn, uint256 scryOut);
    ...
    constructor(IERC20 _scry, IERC20 _coin, ISwapRouter _router, uint24 _convertFee, address _payee, address _steward) {
        require(address(_scry) != address(0) && address(_coin) != address(0), "zero token");
        require(address(_scry) != address(_coin), "scry cannot be the coin");
    ...
    /// the immutable payee.
    function convert(uint256 amountIn, uint256 minScryOut, uint256 deadline)
        external
        onlySteward
        nonReentrant
        returns (uint256 scryOut)
    {
        ...
        emit Converted(amountIn, scryOut);
    }
    ...
    function pay() external nonReentrant returns (uint256 amount) {
        ...
        totalPaid += amount;
        SafeERC20.safeTransfer(address(scry), payee, amount, "pay failed");
    ...
    function pending() external view returns (uint256 coinHeld, uint256 scryHeld) {
        return (coin.balanceOf(address(this)), scry.balanceOf(address(this)));
    }
}
