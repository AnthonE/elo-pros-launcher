...
/// @title ScryCurve — the two-way venue a launch needs before it has a pool
...
///         premise was false, and the hole is in `ScryLaunchpad.seed()`**, which
///         requires the vault to already hold SCRY (`scryAmount > 0`, balance
...
///         nobody runs.
///
///         Operator, 2026-08-13: *"the scry seed needs to come from buys buying
...
///
///         ⚠ WHAT LEAVES, STATED EXACTLY — because `ScryLaunchpad`'s headline is
...
///         must never be described with `ScryLaunchpad`'s sentence.
///
...
///
///         ⚠ IT COMPOSES WITH `ScryLaunchpad` WITH NO CHANGE TO IT. The curve
///         seeds the pool; the vault deepens it. `ScryLaunchpad.seed()` calls
...
contract ScryCurve is ReentrancyGuard {

    string public constant NOTICE =
        "a game coin's launch curve, quoted in SCRY - buy or SELL BACK from the first trade, then at a posted "
        "threshold the curve closes and anyone may graduate it into a uniswap v3 position this contract can "
        "never move. SCRY leaves here three ways and no others: to a seller along the curve, into the locked "
    ...
    /// at deploy, exactly as `ScryLaunchpad` does it and for the same reason: a
    ...
    /// `ScryLaunchpad.pair`, this call moves the caller's OWN money, so a bound
    ...
    /// to anybody. (`ScryLaunchpad.seed` is steward-gated for the opposite reason:
    ...
    /// `ScryLaunchpad.collectFees` and for the same reason: the legs land here
