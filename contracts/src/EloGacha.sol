...
import {RESERVE_BURN} from "./EloFeeSplitter.sol";
...
/// @title EloGacha — a floor under someone else's NFT, and a draw priced off it
...
///         request and revealed unconditionally after it (`EloNotary` already
...
contract EloGacha is ReentrancyGuard {
    ...
    /// @notice Where a burned toll lands. Imported from `EloFeeSplitter` rather
    ...
    ///      The carry is not a nicety: `EloGardener` and `EloSilo` both learned
    ...
    ///      `EloFeeSplitter` this is the whole handoff — the splitter takes it
