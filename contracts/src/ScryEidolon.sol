...
interface IScryTicket {
    function ownerOf(uint256 tokenId) external view returns (address);
    function burnFrom(address from, uint256 tokenId) external;
    function MAX_SUPPLY() external view returns (uint256);
}

/// @title ScryEidolon — the Thousand: 1000 one-of-one familiar vessels
...
///         mint takes a ScryTicket the caller holds and burns it in the act —
...
///         the ScryFeeSplitter remains as a knob when `price > 0` — the launch
...
contract ScryEidolon is ReentrancyGuard {
    string public constant name = "scry eidolons - the thousand";
    string public constant symbol = "EIDOLON";
    string public constant NOTICE = "1000 vessels, one burned ticket each (ScryTicket, same 1000 cap), flat SCRY to the "
    ...
    IScryTicket public immutable ticket; // the one gate: hold a ticket, burn it to summon
