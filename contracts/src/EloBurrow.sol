...
/// @title EloBurrow — the playground's toy lending pool (liquidations are content)
/// @notice Deposit collateral, borrow the debt token against it, keep your
...
///         - The lendable side is donated (transfer) — no lender
...
contract EloBurrow is ReentrancyGuard {
    string public constant NOTICE =
        "toy lending pool - play tokens only, manipulable oracle by design, elo playground";

    IERC20 public immutable collateralToken; // garden token0
    IERC20 public immutable debtToken; // garden token1
    ...
    mapping(address => uint256) public collateral; // collateral posted
    ...
    event Liquidate(address indexed borrower, address indexed liquidator, uint256 debtRepaid, uint256 collateralSeized);
    ...
    function debtOf(address who) public view returns (uint256) {
        uint256 idx = borrowIndex + (borrowIndex * RATE_PER_SEC * (block.timestamp - lastAccrue)) / 1e18;
    ...
    function depositCollateral(uint256 amount) external {
        ...
        SafeERC20.pullExact(address(collateralToken), msg.sender, address(this), amount, "in");
    ...
    function withdrawCollateral(uint256 amount) external {
        ...
        SafeERC20.safeTransfer(address(collateralToken), msg.sender, amount, "out");
    ...
    function borrow(uint256 amount) external nonReentrant {
        ...
        SafeERC20.safeTransfer(address(debtToken), msg.sender, amount, "pool dry - donate the debt token");
    ...
    function repay(uint256 amount) external nonReentrant {
        ...
        // A full repay zeroes the shares outright: the share conversion
        // rounds down, and without this a max-repay strands 1 wei of
        // unpayable dust debt forever (healthFactor never returns max).
        if (pay == debt) debtShares[msg.sender] = 0;
        else debtShares[msg.sender] -= (pay * 1e18) / borrowIndex;
        SafeERC20.pullExact(address(debtToken), msg.sender, address(this), pay, "in");
    ...
    function liquidate(address borrower) external nonReentrant {
        ...
        SafeERC20.pullExact(address(debtToken), msg.sender, address(this), debt, "repay in");
        SafeERC20.safeTransfer(address(collateralToken), msg.sender, seize, "seize out");
