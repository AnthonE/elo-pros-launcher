...
// it BEFORE a splitter exists to ask (`EloFeeSplitter.BURN` is not reachable
// through the contract type in Solidity). The contract re-exposes it as `BURN`
// for on-chain readers.
address constant RESERVE_BURN = 0x000000000000000000000000000000000000dEaD;

/// @title EloFeeSplitter — every SERVICE fee flows through one posted split
...
///         basis-point split — typically bank (EloBank, the xSCRY flywheel) /
...
///         sentence "reserve can enter" (2026-07-25, `SENTENCES.md`) REVERSED that:
...
contract EloFeeSplitter is ReentrancyGuard {
    ...
    ///         already the town's convention (EloGardener's slash ladder).
    ...
    address public constant BURN = RESERVE_BURN;
    ...
    constructor(IERC20 _reserve, address[] memory to, uint16[] memory bps) {
        reserve = _reserve;
        operator = msg.sender;
        _setSplit(to, bps);
    }
    ...
    function distribute() external nonReentrant returns (uint256 amount) {
        ...
        for (uint256 i = 0; i < n; i++) {
            ...
            SafeERC20.safeTransfer(address(reserve), cuts[i].to, share, "transfer failed");
        }
        if (burned > 0) {
            totalBurned += burned;
    ...
    function setSplit(address[] calldata to, uint16[] calldata bps) external onlyOperator {
        if (reserve.balanceOf(address(this)) > 0) this.distribute();
        _setSplit(to, bps);
    }
    ...
    ///         sale. This contract is `EloEidolon`'s royalty receiver and that
    ///         address is IMMUTABLE from its constructor (EloEidolon.sol:64),
    ...
    ///         this `receive()` is the fix. DeployEloEidolon has no such probe.
    ///
