...
/// @title EloArbiter — the staked, metered dispute panel (reference impl)
/// @notice A real `IEloArbiter` for `EloJobBoard`. The whole point of the
...
///         each of them a metered vow (reserve's own instrument turns on the
...
///         `EloJobBoard.dispute` closes the job BEFORE calling `rule`, so
...
contract EloArbiter is IEloArbiter {
