...
/// @title EloTrove — many collections, one pot
/// @notice **The problem, and it is arithmetic rather than taste.** `EloGacha`
...
///         withdraw, no pause, no allowlist and no upgrade path. The one
///         owner door is `metadataOwner` — name/symbol only, and it reaches
///         nothing that is escrowed. The only way an
...
///
///         ⚠ **`transferFrom` is plain and MUST stay plain.** `EloGacha` moves
...
///         may be added.
contract EloTrove is ReentrancyGuard {
    string public constant NOTICE =
        "a wrapper so one gacha pool can hold many collections and donated tokens - no fee, "
        "no pause, and no admin key over what it escrows - name/symbol are settable and "
        "reach nothing else; unwrap is always open and returns exactly what went in";

    string public name = "Elo Trove";
    ...
    struct Held {
        ...
        // APPENDED, following `EloGacha.Position`'s habit even though nothing
    ...
    ///         `EloGacha` never asks a collection for metadata (its
    ...
    ///      `EloGacha`'s toll events are separate: an indexer that only ever
    ...
    ///      shape `EloGacha.deposit` uses — deliberately, because a collection
    ...
    // `EloEidolon` is the template. Nothing here overrides a transfer, and no
