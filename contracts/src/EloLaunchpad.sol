...
/// @title EloLaunchpad — the copy sale's money, with only one door and it leads to the pool
/// @dev formerly `EloLaunch`, renamed 2026-08-13 while still undeployed —
...
///         argument, so a game deploys its own — `EloFeeSplitter` (posted
///         basis points, `BURN` is an ordinary recipient) and `EloBank` (stake
...
contract EloLaunchpad is ReentrancyGuard {
    ...
    IERC20 public immutable reserve; // the reserve, and the only pair asset
    ...
    /// runs both instruments and both are token-generic: `EloFeeSplitter`
    /// routes one token by posted basis points with `BURN` available as an
    /// ordinary recipient, and `EloBank` stakes one token so fee income swells
    ...
    address public immutable reserveFeeRecipient;
    ...
    uint256 public totalEloPaired;
    uint256 public totalCoinPaired;

    event Seeded(address indexed pool, uint256 indexed positionId, uint128 liquidity, uint256 reserveUsed, uint256 coinUsed);
    event Paired(uint256 indexed positionId, uint128 liquidity, uint256 reserveUsed, uint256 coinUsed);
    event Converted(address indexed tokenIn, uint256 amountIn, uint256 reserveOut);
    event Wrapped(uint256 amount);
    event FeesCollected(address indexed reserveTo, address indexed coinTo, uint256 reserveFees, uint256 coinFees);
    ...
    constructor(
        ...
        reserveFeeRecipient = _reserveFeeRecipient;
    ...
    /// has no exit.
    function convert(address tokenIn, uint256 amountIn, uint256 minEloOut, uint256 deadline)
        external
        onlySteward
        nonReentrant
        returns (uint256 reserveOut)
    {
        ...
        emit Converted(tokenIn, amountIn, reserveOut);
    }
    ...
    function seed(uint160 sqrtPriceX96, uint256 reserveAmount, uint256 coinAmount, uint256 deadline)
        external
        onlySteward
        nonReentrant
        returns (uint256 tokenId, uint128 liquidity)
    {
        ...
        require(reserveAmount > 0 && coinAmount > 0, "both sides");
        require(reserve.balanceOf(address(this)) >= reserveAmount, "not enough elo held");
        ...
        (uint256 amount0, uint256 amount1) =
            token0 == address(reserve) ? (reserveAmount, coinAmount) : (coinAmount, reserveAmount);

        SafeERC20.safeApprove(address(reserve), address(positionManager), reserveAmount, "approve failed");
        ...
        SafeERC20.safeApprove(address(reserve), address(positionManager), 0, "approve reset failed");
        SafeERC20.safeApprove(address(coin), address(positionManager), 0, "approve reset failed");

        positionId = tokenId;
        (uint256 reserveUsed, uint256 coinUsed) =
            token0 == address(reserve) ? (used0, used1) : (used1, used0);
        totalEloPaired += reserveUsed;
        totalCoinPaired += coinUsed;
        emit Seeded(pool, tokenId, liquidity, reserveUsed, coinUsed);
    }
    ...
    function pair(uint256 deadline) external nonReentrant returns (uint128 liquidity) {
        ...
        uint256 reserveAmount = reserve.balanceOf(address(this));
        uint256 coinAmount = coin.balanceOf(address(this));
        require(reserveAmount > 0 && coinAmount > 0, "nothing to pair");
        ...
        totalEloPaired += reserveUsed;
        totalCoinPaired += coinUsed;
        emit Paired(positionId, liquidity, reserveUsed, coinUsed);
    }
    ...
    function collectFees() external nonReentrant returns (uint256 reserveFees, uint256 coinFees) {
        require(positionId != 0, "not seeded");
        ...
        (uint256 owedReserve, uint256 owedCoin) = token0 == address(reserve) ? (owed0, owed1) : (owed1, owed0);

        reserveFees = _bounded(reserve.balanceOf(address(this)) - reserveBefore, owedReserve);
        coinFees = _bounded(coin.balanceOf(address(this)) - coinBefore, owedCoin);

        if (reserveFees > 0) SafeERC20.safeTransfer(address(reserve), reserveFeeRecipient, reserveFees, "elo fee pay failed");
        if (coinFees > 0) SafeERC20.safeTransfer(address(coin), coinFeeRecipient, coinFees, "coin fee pay failed");
        emit FeesCollected(reserveFeeRecipient, coinFeeRecipient, reserveFees, coinFees);
    }
    ...
    function pending() external view returns (uint256 ethHeld, uint256 reserveHeld, uint256 coinHeld) {
        return (address(this).balance, reserve.balanceOf(address(this)), coin.balanceOf(address(this)));
    }
