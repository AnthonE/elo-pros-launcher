...
/// @title EloVowRegistry — soulbound vows + merkle-anchored drift ledgers
...
///      reserve meter posts a merkle root over every vow's current chain head.
///      One anchor covers every ledger in existence: after it lands, no one
///      — INCLUDING the reserve operator — can rewrite any anchored history
...
contract EloVowRegistry {
    ...
    // ── anchor state ────────────────────────────────────────────────────────
    address public oracle; // the reserve meter's anchor wallet
    ...
    event VowTaken(
        ...
        bytes32 indexed vowId,
        address indexed swearer,
        bytes32 textHash,
        uint32 cadenceHours,
        bool sealed_,
        string text
    ...
    /// @param vowId    content-addressed id from the reserve API (bytes32-padded)
    ...
    function takeVow(bytes32 vowId, bytes32 textHash, uint32 cadenceHours, bool sealed_, string calldata text)
        external
        returns (uint256 tokenId)
    {
        ...
        tokenId = nextTokenId++;
        vows[tokenId] = Vow(vowId, textHash, cadenceHours, uint64(block.timestamp), sealed_, msg.sender);
    ...
    function _builtinContractURI() internal pure returns (string memory) {
        return string.concat(
            ...
            '{"name":"elo vows","description":"Soulbound vows for AI agents. '
            ...
            'never a verdict. https://elopros.com",'
            '"external_link":"https://elopros.com/verify.html"}'
        );
    }
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        ...
        string memory json = string.concat(
            ...
            v.sealed_
                ? "The text is SEALED (committed by sha256). "
                : "The text is in the VowTaken event log, forever. ",
            'Ledger + trajectory: https://elopros.com/verify.html",',
            '"image":"data:image/svg+xml;base64,',
            _b64(bytes(_svg(idHex, v))),
            '",',
            '"attributes":[',
            '{"trait_type":"vow_id","value":"',
            idHex,
            '"},',
            '{"trait_type":"text_sha256","value":"',
            _hex32(v.textHash),
            '"},',
            '{"trait_type":"cadence_hours","value":',
            _u(v.cadenceHours),
            "},",
            '{"trait_type":"sealed","value":"',
            v.sealed_ ? "true" : "false",
            '"},',
            '{"display_type":"date","trait_type":"taken_at","value":',
            _u(v.takenAt),
            "}",
            "]}"
        );
    ...
    function _svg(string memory idHex, Vow memory v) internal pure returns (string memory) {
        return string.concat(
            ...
            v.sealed_ ? unicode"🔒 sealed vow" : "elo vow",
            "</text>",
            ...
            "every ",
            _u(v.cadenceHours),
            "h &#183; soulbound &#183; elopros.com</text></svg>"
        );
    }
