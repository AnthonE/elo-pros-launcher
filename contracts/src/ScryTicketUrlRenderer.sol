...
contract ScryTicketUrlRenderer {
    /// The origin, with no trailing slash — e.g. `https://scry.moreright.xyz`.
    string public base;
    ...
    /// Move the origin. An empty base is refused rather than stored: it would
    /// produce `/ticket/gates/1/metadata`, a relative path a marketplace
    /// resolves against ITS own host, which 404s somewhere we never see.
    ///
