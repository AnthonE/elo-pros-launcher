...
/// @title EloKiln — a card set goes in, a crest comes out, the cards are gone
/// @notice The craft step of the card layer, and the only new contract that
///         layer needs. Everything else is `EloItem`: a card is an instance,
...
///         `EloGacha` is DEPLOYED and its pools are keyed by **collection
...
///         contract** (`EloItem.setMinter`), and to be **approved by the
...
///         this suite and `EloItem.t.sol`, inside a full 684/684 run, at
...
///         parameters.
contract EloKiln is ReentrancyGuard {
    ...
    /// Matches `EloItem.KIND_CRAFT`. A crest is neither a drop, nor a case,
    ...
    IEloItem public immutable cards;
    IEloItem public immutable crests;
    ...
    constructor(address cards_, address crests_) {
        ...
        cards = IEloItem(cards_);
        crests = IEloItem(crests_);
        owner = msg.sender;
    ...
    function fire(uint32 setId, uint256[] calldata tokenIds) external nonReentrant returns (uint256 crestTokenId) {
        ...
        // The level is inside the token's identity, so `EloItem.mintRefUsed`
