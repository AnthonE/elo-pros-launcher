...
/// An on-chain metadata renderer — `EloSeatArt` is the implementation. Kept as
...
// `EloFeeSplitter`'s: burn by TRANSFER, never by assuming a `burn()` — SCRY is
...
/// @title EloSeat — the founder mint: a character that is a seat
/// @notice The Elo Pros as bytecode. A capped run of characters; mechanically each
...
///         `EloFeeSplitter` would have been less code, but the splitter's
...
///         (`EloSeatArt.traitsOf`, `art/seat/sprites.py`). This header said
...
///         `EloEidolon`'s sealed-salt deck, harvested exactly as designed —
...
contract EloSeat is ReentrancyGuard {
    ...
    string public constant NOTICE = "founder seats - four posted doors (snapshot, play, build, paid) plus a CAPPED treasury mint; "
        ...
        "tier is sublinear, paid in the reserve, part burned, and CLEARS ON TRANSFER; the record is soulbound "
    ...
    /// An on-chain `EloSeatArt`, or address(0) to serve metadata off `baseURI`.
    ...
    uint256 public paidPriceReserve; // 0 = the SCRY leg is shut
    ...
    event PaidDoorSet(uint256 priceWei, uint256 priceReserve);
    ...
    /// @param tierCostReserve CUMULATIVE SCRY per tier, ascending, 18-dec units.
    ...
    constructor(
        ...
        uint256[] memory tierCostReserve,
        ...
        require(tierCostReserve.length > 0, "no ladder");
        require(tierCostReserve.length == tierWeightX100.length, "ladder mismatch");
        require(tierCostReserve.length <= 255, "ladder too deep"); // tierOf is uint8
        require(tierCostReserve[0] > 0 && tierWeightX100[0] > 0, "zero base tier");
        ...
        for (uint256 i = 1; i < tierCostReserve.length; i++) {
            require(tierCostReserve[i] > tierCostReserve[i - 1], "ladder cost not ascending");
            require(tierWeightX100[i] > tierWeightX100[i - 1], "ladder weight not ascending");
            require(
                uint256(tierWeightX100[i]) * tierCostReserve[0] < uint256(tierWeightX100[0]) * tierCostReserve[i],
    ...
    function setPaidDoor(uint256 priceWei, uint256 priceReserve) external onlyOwner {
        paidPriceWei = priceWei;
        paidPriceReserve = priceReserve;
        emit PaidDoorSet(priceWei, priceReserve);
    }
    ...
    function buyWithReserve() external nonReentrant returns (uint256) {
        require(paidPriceReserve > 0, "elo leg shut");
        require(paidMinted < _paidCeiling(), "paid door spent");
        paidMinted++;
        SafeERC20.safeTransferFrom(address(reserve), msg.sender, feeSplitter, paidPriceReserve, "elo payment failed");
        return _mint(msg.sender, 4);
    }
    ...
    function activate(uint256 tokenId, uint8 tier) external nonReentrant {
        ...
        if (burned > 0) {
            ...
            SafeERC20.safeTransferFrom(address(reserve), msg.sender, BURN, burned, "burn leg failed");
        }
        uint256 toSplitter = owed - burned;
        if (toSplitter > 0) {
            SafeERC20.safeTransferFrom(address(reserve), msg.sender, feeSplitter, toSplitter, "splitter leg failed");
        }
        emit Activated(tokenId, tier, owed, burned);
        // ⚠ ERC-4906, AND IT IS NOT DECORATION HERE. `EloSeatArt` renders tier
    ...
    // ── metadata ─────────────────────────────────────────────────────────────
    /// Two sources, in order: an on-chain `EloSeatArt` if one is set, else the
