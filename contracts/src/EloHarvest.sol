...
/// @title EloHarvest — merkle claims against the public Augury harvest ledger
...
///         EloVowRegistry.verifyProof, so the whole suite shares one merkle
...
contract EloHarvest is ReentrancyGuard {
    ...
    function claim(address wallet, uint256 cumulative, bytes32[] calldata proof) external nonReentrant returns (uint256 amount) {
        ...
        SafeERC20.safeTransfer(address(reserve), wallet, amount, "transfer failed");
    ...
    function sweep(address to, uint256 amount) external onlyOperator nonReentrant {
        ...
        SafeERC20.safeTransfer(address(reserve), to, amount, "transfer failed");
    ...
    /// Same sorted-pair keccak256 dialect as EloVowRegistry.verifyProof.
