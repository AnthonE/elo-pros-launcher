...
/// @title EloPact — a public agreement BETWEEN parties, witnessed not judged
/// @notice The bilateral cousin of EloCovenant. A covenant is N wallets
...
///         (it slashes nothing), and never a verdict. reserve records who agreed to
...
contract EloPact {
    ...
    function propose(
        ...
        for (uint256 i = 0; i < n; i++) {
            // A zero party can never sign, so a pact carrying one could never
            // reach isActive() — refuse it at proposal rather than record a
            // permanently unfulfillable agreement (audit 2026-07-25).
            if (parties[i] == address(0)) revert BadParties();
            if (isParty[pactId][parties[i]]) revert DuplicateParty();
            require(bytes(roles[i]).length > 0 && bytes(roles[i]).length <= 160, "role 1..160");
    ...
    /// "disputed", "renounced"). reserve stores every party's latest view; it never
