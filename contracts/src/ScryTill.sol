...
/// @title ScryTill — signed withdrawals against the off-chain play ledger
...
///
///         Token-generic, one instance per coin, exactly like `ScryHarvest`:
...
contract ScryTill is ReentrancyGuard {
    ...
    /// dialect already in the tree — `ScryHarvest.claimed[wallet]` is the same
    ...
    bytes32 private constant NAME_HASH = keccak256("ScryTill");
    ...
    /// exists — the check `ScryPeculium._recover` already carries.
    ...
    /// `ScryHarvest.SWEEP_DELAY`, so the suite has one duration rather than
    ...
    /// never to `msg.sender`. The property `ScryHarvest.claim` already has, for
    ...
    function withdraw(Draft calldata d, bytes calldata sig) external nonReentrant returns (uint256 paid) {
        ...
        // days. `ScryGacha` already shipped a ~20-minute reveal that was meant
    ...
    /// The float's own door. `ScryHarvest.sweep` can bound itself with
    ...
    /// the role to nobody — the failure `ScryGranary`'s steward handoff was
