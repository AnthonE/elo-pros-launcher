...
/// @title ScryGardener - the LP farm (Master Gardener homage, FARMING.md)
...
///         Gardens mechanic with scry's one structural correction: the coin
...
contract ScryGardener is ReentrancyGuard {
    // -- accounting ---------------------------------------------------------
    struct UserInfo {
    ...
    ScryGranary public immutable granary; // MYRRH mints go through here
    ...
    constructor(ScryGranary _granary, uint256 _rewardPerSecond, uint256 _lockBps, uint256 _unlockTime) {
    ...
    // The granary call is try/catch on purpose (the ScrySilo rule): a revoked
    ...
    function deposit(uint256 pid, uint256 amount) external nonReentrant {
        ...
        if (amount > 0) {
            ...
            // ScryBank fix; also the second half of the codeless-LP guard in
    ...
    function claimLocked() external nonReentrant returns (uint256 paid) {
        ...
        // `ScrySilo.claimOwed` (audit 2026-07-27). This was the cleanest of the
