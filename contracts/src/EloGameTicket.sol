...
/// Optional external metadata renderer, same shape `EloDeed` already uses.
/// If the owner sets one it answers both URIs; `address(0)` restores the
/// built-in on-chain metadata.
interface IEloTicketRenderer {
    function tokenURI(uint256 tokenId) external view returns (string memory);
    function contractURI() external view returns (string memory);
}

/// @title EloGameTicket — a copy of a game, as a token you own
...
///         every surface that already reads `priceWei()` / `priceReserve()` /
...
///           - SCRY   → `reserveSink`, straight from the buyer; this contract
///             never holds a wei of SCRY. Point it at the deployed
///             EloFeeSplitter and the posted split applies — half of every
///             SCRY buy burns, and no burn is claimed until `distribute()` has
///             actually run. Point it at a `EloLaunchpad` and the SCRY leg is
...
///         — a url on reserve's own origin — which made the ONE bearer asset on
...
///         document from contract storage, exactly as `EloDeed` already does
...
contract EloGameTicket is ReentrancyGuard {
    /// The TITLE, bare — "Gates", not "Gates on Elo Pros" and not "Gates copy".
    ...
    ///
    /// The collection reads **"<title> on Elo Pros"** rather than *by* — a
    /// storefront credit is true for every listing, and most listings are not
    /// ours to take authorship of. The credit is SETTABLE rather than welded:
    /// this platform has been renamed more than once, and a constant here put
    /// whichever name was current on broadcast day into every listing forever.
    ...
    string public platform = "Elo Pros";

    // ⚠ THIS STRING USED TO CLAIM THE BURN, AND IT COULD NOT KEEP THE PROMISE.
    // It read "paid in SCRY (half burns via the posted split)" — but `reserveSink`
    ...
    string public constant NOTICE = "the game itself, as a token - holding any ticket is what the official "
        ...
        "each rail's money goes is welded at deploy - read reserveSink and proceeds and check what they are; free "
    ...
    address public immutable reserveSink; // the fee splitter — SCRY buys land here, half burns by the posted split
    ...
    //
    // `baseURI` is the SITE base (e.g. "https://elopros.com") and is
    ...
    uint256 public constant RAIL_ELO = 2;
    ...
    // Elo Pros holders" through the root meant a snapshot, a proofs file the
    ...
    mapping(uint256 => uint256) public railOf; // 1 eth · 2 reserve · 3 usdg · 4 comp · 5 free · 6+ custom
    ...
    event PricesPosted(uint256 usdCents, uint256 ethWei, uint256 reserveWei, uint256 usdgUnits);
    event RailPosted(
    ...
    constructor(
        ...
        IERC20 _reserve,
        IERC20 _usdg,
        address _reserveSink,
        address _proceeds,
        uint256 _supplyCap,
        string memory _baseURI,
        address _owner
    ) {
        require(address(_reserve) != address(0) && address(_usdg) != address(0), "zero coin");
        require(_reserveSink != address(0), "zero sink");
        ...
        require(_reserveSink != address(this), "sink is this contract");
    ...
    function setPrices(uint256 usdCents, uint256 ethWei, uint256 reserveWei, uint256 usdgUnits) external onlyOwner {
        priceUsdCents = usdCents;
        _post(RAIL_ETH, ethWei);
        _post(RAIL_ELO, reserveWei);
        _post(RAIL_USDG, usdgUnits);
        emit PricesPosted(usdCents, ethWei, reserveWei, usdgUnits);
    }
    ...
    /// — closing rail 2 is the same act as posting `priceReserve` at 0.
    ...
    // that reads `priceWei()` / `priceReserve()` / `priceUsdg()` — the meter card,
    ...
    function priceReserve() external view returns (uint256) {
        return _amountIfOpen(RAIL_ELO);
    }
    ...
    /// The storefront credit every collection name is built from. Settable
    /// because a constant here welds a dead brand into every listing: the
    /// previous generation shipped with a platform name three renames stale.
    function setPlatform(string calldata newPlatform) external onlyOwner {
        platform = newPlatform;
        emit ContractURIUpdated();
        emit BatchMetadataUpdate(1, type(uint256).max);
    }

    /// Rename the title. The collection a marketplace shows is built as
    /// `<name> on <platform>`, so this is the title's half of that and
    /// `setPlatform` is ours. The game's own team holds this contract's owner.
    ...
    /// `EloTicketUrlRenderer` the document lives on our origin, so editing the
    ...
    /// EloTicket and the harvest drops, so the meter's existing root pipeline
    ...
    // `EloDeed`'s, which shipped the same reasoning first: build the document
    // from storage, base64 it, hand back a `data:` URI. A viewer needs no
    // server, no gateway and no cooperation from reserve to render a licence its
    // holder paid for.

    function tokenURI(uint256 tokenId) external view returns (string memory) {
        ownerOf(tokenId); // reverts for a token that does not exist
        if (renderer != address(0)) return IEloTicketRenderer(renderer).tokenURI(tokenId);
    ...
    function contractURI() external view returns (string memory) {
        if (renderer != address(0)) return IEloTicketRenderer(renderer).contractURI();
        string memory json = string.concat(
            '{"name":"',
            _esc(name),
            " on ",
    ...
    /// suite that only checked the token. `EloDeed` and `EloSeatArt` both
