...
///   #   rather than a shrug. Unlike the pass, `ScryItem` welds BOTH the
...
///   # export CARD_BASE_URI=https://scry.moreright.xyz/api/items/cards
///   # export CREST_BASE_URI=https://scry.moreright.xyz/api/items/crests
///   # a first set, defined in the same broadcast (skipped when unset).
///   # ⚠ These must MATCH data/cards/catalogs.json — the chain is what
///   #   ScryKiln freezes, the catalog is only what names it. Gates set one:
///   # export SET_ID=1
...
contract DeployCardLayer is Script {
    function run() external {
        ...
        string memory cardBaseURI =
            vm.envOr("CARD_BASE_URI", string("https://scry.moreright.xyz/api/items/cards"));
        string memory crestBaseURI =
            vm.envOr("CREST_BASE_URI", string("https://scry.moreright.xyz/api/items/crests"));
        ...
        ScryItem cards = new ScryItem(cardName, cardSymbol, cardReceiver, uint96(cardBps), cardBaseURI);
        ScryItem crests = new ScryItem(crestName, crestSymbol, crestReceiver, uint96(crestBps), crestBaseURI);
        ScryKiln kiln = new ScryKiln(address(cards), address(crests));
        ...
        console2.log("ScryItem (cards) ", address(cards));
        console2.log("ScryItem (crests)", address(crests));
        console2.log("ScryKiln         ", address(kiln));
