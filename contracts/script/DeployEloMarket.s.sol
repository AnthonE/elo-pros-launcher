...
///   export SCRY_FEE_SPLITTER=0x…     # existing EloFeeSplitter
///   export SCRY_ARBITER=0x…          # a deployed IEloArbiter (panel)
...
///   forge script script/DeployEloMarket.s.sol \
...
///
///   arbiter flatFee   15,000 SCRY  ≈ $0.52   (DeployEloArbiter.s.sol)
...
contract DeployEloMarket is Script {
    function run() external {
        ...
        EloReputation reputation = new EloReputation(threshold);
        EloInsurancePool pool = new EloInsurancePool(reserve, maxPayout);
        EloJobBoard board = new EloJobBoard(
            reserve, IReputation(address(reputation)), IInsurancePool(address(pool)), feeSplitter, arbiter, maxArbFee
        );
        ...
        console.log("EloReputation", address(reputation));
        console.log("EloInsurancePool", address(pool));
        console.log("EloJobBoard", address(board));
    ...
    /// arbiter may be a third-party IEloArbiter with no such function). A
