...
///   forge script script/DeployScryEidolon.s.sol \
...
///   - mintGacha(<house wallet>, n) then escrow each ticket into ScryGacha
...
contract DeployScryEidolon is Script {
    function run() external {
        vm.startBroadcast(vm.envUint("PRIVATE_KEY"));
        ScryTicket tick = new ScryTicket(
        ...
        ScryEidolon eid = new ScryEidolon(
            IERC20(vm.envAddress("SCRY_TOKEN")),
            IScryTicket(address(tick)),
        ...
        vm.stopBroadcast();
        console2.log("ScryTicket ", address(tick));
        console2.log("ScryEidolon", address(eid));
    }
}
