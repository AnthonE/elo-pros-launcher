...
/// Deploy the dispute panel (IScryArbiter) that ScryJobBoard rules through.
///
...
///   forge script script/DeployScryArbiter.s.sol \
...
/// default in DeployScryMarket.s.sol) or every dispute reverts.
///
...
/// would settle on it instantly. `ScryArbiter.setBoard` is now a ONE-TIME,
...
contract DeployScryArbiter is Script {
    function run() external {
        ...
        ScryArbiter arbiter = new ScryArbiter(feeRecipient, flatFee, panel, quorum);
        vm.stopBroadcast();

        console.log("ScryArbiter", address(arbiter));
