...
///   forge script script/DeployEloVowRegistry.s.sol \
...
contract DeployEloVowRegistry is Script {
    function run() external {
        uint256 pk = vm.envUint("PRIVATE_KEY");
        address oracle = vm.envOr("SCRY_ANCHOR_ORACLE", vm.addr(pk));
        vm.startBroadcast(pk);
        EloVowRegistry reg = new EloVowRegistry(oracle);
        vm.stopBroadcast();
        console2.log("EloVowRegistry deployed:", address(reg));
