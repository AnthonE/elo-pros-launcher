...
/// Deploy EloDeed — the tabula: a house on a named street, as a REGULAR NFT
/// that is also a sink.
///
...
///   forge script script/DeployEloDeed.s.sol \
...
/// is NOT here: it stays soulbound to the vow at EloReputation and never moves
/// with a deed. Written in an env without foundry — `forge test -vv` first.
contract DeployEloDeed is Script {
    function run() external {
        ...
        string memory baseURI = vm.envOr("BASE_URI", string("https://elopros.com/api"));
        vm.startBroadcast(pk);
        EloDeed deed = new EloDeed(ISpoils(obol), foundingBurn, conveyanceBurn, baseURI);
        vm.stopBroadcast();
        console2.log("EloDeed", address(deed));
