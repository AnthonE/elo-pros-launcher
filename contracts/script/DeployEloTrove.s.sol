...
import "../src/EloTrove.sol";

/// Deploy the shared pot's wrapper (`EloTrove.sol`'s header is the design;
/// `contracts/RUNBOOK.md` is the deploy order).
///
/// **ONE deployment, ever — and this is the opposite of `DeployEloItem`.**
...
/// entire function**: `EloGacha` keys pools by collection, so everything that
...
///   # optional:
///   # export TROVE_BASE_URI=https://elopros.com/api/trove/
///
///   forge script script/DeployEloTrove.s.sol \
...
contract DeployEloTrove is Script {
    function run() external {
        uint256 pk = vm.envUint("PRIVATE_KEY");
        string memory baseURI = vm.envOr("TROVE_BASE_URI", string(""));

        vm.startBroadcast(pk);
        EloTrove trove = new EloTrove(baseURI);
        vm.stopBroadcast();

        console2.log("EloTrove      ", address(trove));
