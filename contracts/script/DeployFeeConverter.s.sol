...
/// launch vault.
///
/// WHY. `EloLaunchpad.collectFees()` pays whatever the v3 position owes, which is
...
/// `LAUNCHPAD_FEE_RECIPIENT` when deploying `EloLaunchpad` — that field is immutable,
...
/// to `EloLaunchpad`, which has no way to pay a person at all — that is the whole
/// design, and routing the raise through this contract would undo it.
///
///   export PRIVATE_KEY=0x...
///   export FEE_SCRY=0x...        # chains.4663.contracts.ELO -- NOT .SCRY,
...
contract DeployFeeConverter is Script {
    function run() external {
        address reserve = vm.envAddress("FEE_SCRY");
        ...
        EloFeeConverter conv = new EloFeeConverter(
            IERC20(reserve), IERC20(coin), ISwapRouter(router), uint24(convertFee), payee, steward
        );
        vm.stopBroadcast();

        console2.log("EloFeeConverter", address(conv));
