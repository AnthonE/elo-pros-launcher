...
///
///   1. this script                                  -> EloLaunchpad
///   2. DeployGameTicket.s.sol with
///      TICKET_PROCEEDS=<the vault>                  -> EloGameTicket
...
///
/// ⚠ THE SCRY SINK IS A DIFFERENT PROMISE. Point the ticket's `reserveSink` at the
...
/// a card that says "half burns" over the second is false. `EloGameTicket`'s
/// NOTICE no longer claims either, on purpose.
///
///   export PRIVATE_KEY=0x...
///   export LAUNCHPAD_SCRY=0x...          # chains.4663.contracts.ELO -- NOT .SCRY,
...
///
/// TWO FEE LEGS, because a v3 position pays two tokens and a `EloFeeSplitter`
/// handles one. The coin leg is where a game's own economy hangs: point it at a
/// splitter the studio deploys for its own coin, with `RESERVE_BURN` as one
/// recipient and its own `EloBank` as another, and fee income burns and banks
...
contract DeployLaunchpad is Script {
    function run() external {
        address reserve = vm.envAddress("LAUNCHPAD_SCRY");
        ...
        address reserveFees = vm.envAddress("LAUNCHPAD_SCRY_FEES");
        ...
        EloLaunchpad launch = new EloLaunchpad(
            IERC20(reserve),
            IERC20(coin),
            IWETH9(weth),
        ...
        console2.log("EloLaunchpad", address(launch));
        console2.log("  pass this as TICKET_PROCEEDS - the ticket's proceeds is IMMUTABLE");
        console2.log("  reserve fee leg IMMUTABLE", reserveFees);
