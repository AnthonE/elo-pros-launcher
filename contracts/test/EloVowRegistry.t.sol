...
import "../src/EloVowRegistry.sol";

contract EloVowRegistryTest is Test {
    EloVowRegistry reg;
    ...
    function test_takeVow_wrongHash_reverts() public {
        ...
        vm.prank(agent);
        vm.expectRevert(EloVowRegistry.BadTextHash.selector);
    ...
    function test_takeVow_sealed_withText_reverts() public {
        bytes32 h = sha256(bytes(TEXT));
        vm.prank(agent);
        vm.expectRevert(EloVowRegistry.SealedNeedsNoText.selector);
    ...
    function test_takeVow_duplicate_reverts() public {
        ...
        bytes32 h = sha256(bytes(TEXT));
        vm.prank(stranger);
        vm.expectRevert(EloVowRegistry.VowAlreadyTaken.selector);
        reg.takeVow(vowId, h, 24, false, TEXT);
    }
    ...
    function test_locked_and_transfers_revert() public {
        ...
        vm.startPrank(agent);
        vm.expectRevert(EloVowRegistry.Soulbound.selector);
    ...
    function test_setName_setSymbol_onlyOwner() public {
        reg.setName("reserve oaths");
        reg.setSymbol("OATH");
        assertEq(reg.name(), "reserve oaths");
        assertEq(reg.symbol(), "OATH");
        vm.prank(stranger);
        vm.expectRevert(EloVowRegistry.NotOwner.selector);
        reg.setName("hijack");
    }
    ...
    function test_anchor_onlyOracle() public {
        vm.prank(stranger);
        vm.expectRevert(EloVowRegistry.NotOracle.selector);
    ...
    function test_tokenURI_shape() public {
        ...
        vm.expectRevert(EloVowRegistry.NoSuchToken.selector);
        reg.tokenURI(999);
    }
    ...
    function test_takeVow_emitsMintLockAndFullText() public {
        ...
        emit EloVowRegistry.Transfer(address(0), agent, 1); // the mint
        vm.expectEmit(false, false, false, true);
        emit EloVowRegistry.Locked(1); // ERC-5192
        vm.expectEmit(true, true, true, true);
        emit EloVowRegistry.VowTaken(1, vowId, agent, h, 24, false, TEXT); // full text
        vm.prank(agent);
        reg.takeVow(vowId, h, 24, false, TEXT);
    }
    ...
    function test_anchored_emitsFullPayload() public {
        ...
        emit EloVowRegistry.Anchored(root, 3, "anchor_leaves/20260722T000000.json", 5000);
        vm.prank(oracle);
        reg.anchorRoot(root, 3, "anchor_leaves/20260722T000000.json");
    }

    // ── the actually-deployed configuration ─────────────────────────────────
    // DeployEloVowRegistry.s.sol defaults SCRY_ANCHOR_ORACLE to the deployer,
    ...
    function test_liveConfig_deployerIsOwnerAndOracle() public {
        ...
        vm.prank(deployer);
        EloVowRegistry live = new EloVowRegistry(deployer);
        ...
        vm.prank(deployer);
        live.setName("reserve vows live");
        assertEq(live.name(), "reserve vows live");
    }
    ...
    function test_oracleRotation_newGainsOldLoses() public {
        ...
        emit EloVowRegistry.OracleTransferred(oracle, newOracle);
        vm.prank(oracle);
...
contract MockRenderer is IEloRenderer {
