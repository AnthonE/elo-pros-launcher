...
///   forge test --match-contract EloKilnTest -vv
contract EloKilnTest is Test {
    EloItem cards;
    EloItem crests;
    EloKiln kiln;
    ...
    function setUp() public {
        ...
        cards = new EloItem("Gates Cards", "GCARD", payout, 0, "https://elopros.com/cards/");
        crests = new EloItem("Gates Crests", "GCREST", payout, 0, "https://elopros.com/crests/");
        kiln = new EloKiln(address(cards), address(crests));
    ...
    function test_constructor_refuses_one_contract_for_both() public {
        vm.expectRevert(EloKiln.OneContractForBoth.selector);
        new EloKiln(address(cards), address(cards));
    }

    function test_constructor_refuses_zero() public {
        vm.expectRevert(EloKiln.ZeroAddress.selector);
        new EloKiln(address(0), address(crests));
        vm.expectRevert(EloKiln.ZeroAddress.selector);
        new EloKiln(address(cards), address(0));
    }
    ...
    function test_only_owner_defines() public {
        vm.prank(alice);
        vm.expectRevert(EloKiln.NotOwner.selector);
    ...
    function test_rejects_a_set_that_names_a_card_twice() public {
        ...
        dupes[2] = 1;
        vm.prank(creator);
        vm.expectRevert(EloKiln.DuplicateCatalog.selector);
    ...
    function test_rejects_bad_sizes_and_zero_levels() public {
        ...
        one[0] = 1;
        vm.prank(creator);
        vm.expectRevert(EloKiln.BadSetSize.selector);
        ...
        vm.prank(creator);
        vm.expectRevert(EloKiln.BadMaxLevel.selector);
    ...
    function test_level_cap_binds() public {
        ...
        vm.prank(alice);
        vm.expectRevert(EloKiln.LevelCapReached.selector);
        kiln.fire(SET, sixth);
    }
    ...
    function test_refuses_an_unknown_set() public {
        uint256[] memory ids = _fullSet(alice, 1);
        vm.prank(alice);
        vm.expectRevert(EloKiln.SetUnknown.selector);
        kiln.fire(99, ids);
    }
    ...
    function test_refuses_the_wrong_count() public {
        ...
        vm.prank(alice);
        vm.expectRevert(EloKiln.WrongCardCount.selector);
        kiln.fire(SET, four);
    }
    ...
    function test_refuses_cards_you_do_not_own() public {
        ...
        vm.prank(alice);
        vm.expectRevert(EloKiln.NotYourCard.selector);
        kiln.fire(SET, his);
    ...
    function test_refuses_a_card_from_outside_the_set() public {
        ...
        ids[4] = stranger;
        vm.prank(alice);
        vm.expectRevert(EloKiln.CardNotInSet.selector);
        kiln.fire(SET, ids);
    }
    ...
    function test_refuses_two_of_the_same_card() public {
        ...
        vm.prank(alice);
        vm.expectRevert(EloKiln.DuplicateCard.selector);
        kiln.fire(SET, ids);
    }
    ...
    function test_a_set_freezes_the_first_time_it_fires() public {
        ...
        vm.prank(creator);
        vm.expectRevert(EloKiln.SetIsFrozen.selector);
