...
contract ScryLaunchTest is Test {
    ScryLaunch vault;
    MockToken scry;
    MockToken coin;
    MockToken usdg;
    ...
    address scryFeeTo = address(0xFEE5); // in practice: the studio, or a splitter
    address coinFeeTo = address(0xFEE6); // in practice: the game's OWN ScryFeeSplitter
    ...
    function setUp() public {
        ...
        npm.setTokens(t0, t1);
        factory.setPool(address(scry), address(coin), POOL_FEE, address(pool));
    }

    function _fund(uint256 scryAmt, uint256 coinAmt) internal {
        scry.mint(address(vault), scryAmt);
    ...
    function test_the_steward_cannot_move_the_raise_anywhere_but_the_pool() public {
        _fund(1000e18, 5000e18);
        uint256 before = scry.balanceOf(steward);
        // every steward-callable path, run by the steward, moves nothing to them
        vm.startPrank(steward);
        vault.seed(uint160(1) << 96, 1000e18, 5000e18, block.timestamp + 1);
        vm.stopPrank();
        assertEq(scry.balanceOf(steward), before, "the steward gained nothing");
        assertEq(scry.balanceOf(address(vault)), 0, "and it went to the position");
        assertGt(npm.totalAmount0() + npm.totalAmount1(), 0);
    }

    function test_each_fee_leg_goes_to_its_own_welded_destination() public {
        _seed();
        (address t0,) = address(scry) < address(coin)
            ? (address(scry), address(coin)) : (address(coin), address(scry));
        // owed0/owed1 are in pool ordering; give the scry leg 3 and the coin leg 7
        if (t0 == address(scry)) npm.setFeesOwed(3e18, 7e18);
        else npm.setFeesOwed(7e18, 3e18);

        vm.prank(stranger); // permissionless: both destinations are immutable
        (uint256 scryFees, uint256 coinFees) = vault.collectFees();

        assertEq(scryFees, 3e18);
        assertEq(coinFees, 7e18);
        assertEq(scry.balanceOf(scryFeeTo), 3e18, "the reserve leg");
        assertEq(coin.balanceOf(coinFeeTo), 7e18, "the coin leg - a game's own splitter/bank");
        assertEq(vault.scryFeeRecipient(), scryFeeTo);
    ...
    function test_a_raise_sitting_in_the_vault_is_never_paid_out_as_fees() public {
        ...
        (address t0,) = address(scry) < address(coin)
            ? (address(scry), address(coin)) : (address(coin), address(scry));
        if (t0 == address(scry)) npm.setFeesOwed(1e18, 2e18);
        else npm.setFeesOwed(2e18, 1e18);

        (uint256 scryFees, uint256 coinFees) = vault.collectFees();

        assertEq(scryFees, 1e18, "only what the position owed");
        assertEq(coinFees, 2e18);
        assertEq(scry.balanceOf(scryFeeTo), 1e18);
        assertEq(coin.balanceOf(coinFeeTo), 2e18);
        // and the raise is still here, still poolable
        assertEq(scry.balanceOf(address(vault)), 500e18, "the raise did not move");
    ...
    function test_collecting_nothing_moves_nothing() public {
        _seed();
        vault.collectFees();
        assertEq(scry.balanceOf(scryFeeTo), 0);
    ...
    function test_the_pool_opens_at_seed_with_no_threshold_to_reach() public {
        ...
        assertEq(vault.totalScryPaired(), 1000e18);
    ...
    function test_every_later_sale_deepens_the_same_position() public {
        ...
        assertEq(vault.totalScryPaired(), 1100e18);
        assertEq(scry.balanceOf(address(vault)), 0, "nothing is left sitting");
    }
    ...
    function test_seeding_without_the_coin_side_funded_refuses() public {
        scry.mint(address(vault), 1000e18);
        vm.prank(steward);
    ...
    function test_convert_output_lands_in_the_vault_never_the_caller() public {
        ...
        assertEq(out, 100e18);
        assertEq(scry.balanceOf(address(vault)), 100e18, "the swap cannot pay the steward");
    ...
    function test_convert_refuses_the_two_assets_that_are_not_the_raise() public {
        vm.startPrank(steward);
        vm.expectRevert(bytes("already scry"));
        vault.convert(address(scry), 1, 1, block.timestamp + 1);
    ...
    function test_a_launch_cannot_pair_scry_with_scry() public {
        vm.expectRevert(bytes("scry cannot pair with itself"));
        new ScryLaunch(
            IERC20(address(scry)), IERC20(address(scry)), IWETH9(address(weth)),
            ILaunchPositionManager(address(npm)), ISwapRouter(address(router)),
            POOL_FEE, POOL_FEE, scryFeeTo, coinFeeTo, steward
        );
    }
    ...
    function test_an_unknown_fee_tier_refuses_rather_than_minting_a_bad_range() public {
        ...
        new ScryLaunch(
            IERC20(address(scry)), IERC20(address(coin)), IWETH9(address(weth)),
            ILaunchPositionManager(address(npm)), ISwapRouter(address(router)),
            uint24(1234), POOL_FEE, scryFeeTo, coinFeeTo, steward
        );
    }

    function test_the_range_is_full_and_aligned_to_the_tiers_spacing() public view {
        assertEq(vault.tickUpper(), (887272 / 200) * 200);
        assertEq(vault.tickLower(), -((887272 / 200) * 200));
    }
    ...
    function test_a_copy_sale_becomes_liquidity_end_to_end() public {
        ...
        scry.mint(address(vault), 1_000e18);
        vm.prank(steward);
        ...
        uint256 pairedBefore = vault.totalScryPaired();
        vm.prank(stranger);
        vault.pair(block.timestamp + 1);

        assertEq(vault.totalScryPaired(), pairedBefore + 10e18);
        assertEq(usdg.balanceOf(address(vault)), 0);
        assertEq(scry.balanceOf(address(vault)), 0);
