...
contract EloMarketTest is Test {
    ...
    function setUp() public {
        ...
        reputation = new EloReputation(50); // threshold
    ...
    function testEscrowCompletePaysSellerMinusFeeAndEarnsRep() public {
        ...
        assertEq(reserve.balanceOf(splitter), 5e18);
    ...
    function testSellerDefaultRefundsBuyerAndSlashes() public {
        ...
        board.close(id);
        assertEq(reserve.balanceOf(buyer), 1_000e18); // fully refunded
    ...
    function testBuyerSilentOnDeliveredReleasesToSeller() public {
        ...
        vm.warp(block.timestamp + 2 days);
        board.close(id);
        assertEq(reserve.balanceOf(seller), 1_000e18 + 95e18);
    ...
    function testArbiterFeeIsIndependentOfVerdict() public {
        ...
        arbiter.setVerdict(IEloArbiter.Verdict.ForSeller);
        vm.startPrank(buyer);
        reserve.approve(address(board), 2e18);
        board.dispute(id1);
        vm.stopPrank();
        uint256 feeAfterCase1 = reserve.balanceOf(arbFeeSink);

        // case 2: rules for buyer — same flat fee lands
        uint256 id2 = _postEscrow(100e18);
        arbiter.setVerdict(IEloArbiter.Verdict.ForBuyer);
        vm.startPrank(buyer);
        reserve.approve(address(board), 2e18);
        board.dispute(id2);
        vm.stopPrank();
        uint256 feeAfterCase2 = reserve.balanceOf(arbFeeSink);
    ...
    function testRulingForBuyerRefundsAndSlashes() public {
        ...
        board.dispute(id);
        vm.stopPrank();
        assertEq(reserve.balanceOf(buyer), 1_000e18 - 2e18); // refunded amount, minus the arb fee it paid
    ...
    function testRepGateBlocksLowRepSellerFromRepOnlyJob() public {
        ...
        board.post(seller, 10e18, EloJobBoard.Mode.ReputationOnly, SPEC, uint64(block.timestamp + 1 days), 0);
        vm.stopPrank();
    }
    ...
    function testInsuredModeIsClosedOnAFreshBoard() public {
        EloJobBoard fresh = new EloJobBoard(
            IERC20(address(reserve)),
            IReputation(address(reputation)),
            IInsurancePool(address(pool)),
            splitter,
        ...
        vm.startPrank(buyer);
        reserve.approve(address(fresh), type(uint256).max);
        vm.expectRevert(bytes("insured mode is closed"));
        fresh.post(seller, 100e18, EloJobBoard.Mode.Insured, SPEC, uint64(block.timestamp + 1 days), 3e18);
        // the other two modes are NOT gated by this — the door is on Insured only
        fresh.post(seller, 100e18, EloJobBoard.Mode.Escrow, SPEC, uint64(block.timestamp + 1 days), 0);
        fresh.post(seller, 100e18, EloJobBoard.Mode.ReputationOnly, SPEC, uint64(block.timestamp + 1 days), 0);
        vm.stopPrank();
    }
    ...
    function testInsuredOpenIsOperatorOnlyAndReclosable() public {
        ...
        reputation.earn(seller, 100, "seed");
        vm.startPrank(buyer);
        reserve.approve(address(board), type(uint256).max);
        vm.expectRevert(bytes("insured mode is closed"));
        board.post(seller, 100e18, EloJobBoard.Mode.Insured, SPEC, uint64(block.timestamp + 1 days), 3e18);
        vm.stopPrank();
    }
    ...
    function testInsuredCompleteWithoutFundingReverts() public {
        ...
        reputation.earn(seller, 100, "seed");
        vm.startPrank(buyer);
        reserve.approve(address(board), 3e18); // premium only, amount NOT approved
        uint256 id = board.post(seller, 100e18, EloJobBoard.Mode.Insured, SPEC, uint64(block.timestamp + 1 days), 3e18);
        vm.stopPrank();
        vm.prank(seller);
        board.deliver(id, DELIV);
        vm.prank(buyer);
    ...
    function testInsuredCompleteWithFundingPaysSeller() public {
        ...
        reputation.earn(seller, 100, "seed");
        vm.startPrank(buyer);
        reserve.approve(address(board), 103e18); // premium + amount
        uint256 id = board.post(seller, 100e18, EloJobBoard.Mode.Insured, SPEC, uint64(block.timestamp + 1 days), 3e18);
        vm.stopPrank();
        vm.prank(seller);
        board.deliver(id, DELIV);
    ...
    function testInsuredBuyerDefaultCoversSellerFromPool() public {
        reputation.setAuthority(address(this), true);
        reputation.earn(seller, 100, "seed");
        reserve.mint(address(this), 500e18);
        reserve.approve(address(pool), 500e18);
        pool.payPremium(500e18); // fund the pool

        vm.startPrank(buyer);
        reserve.approve(address(board), 3e18); // premium only; amount is NOT escrowed
        uint256 id = board.post(seller, 100e18, EloJobBoard.Mode.Insured, SPEC, uint64(block.timestamp + 1 days), 3e18);
        vm.stopPrank();
        vm.prank(seller);
        board.deliver(id, DELIV);
    ...
    function testRepOnlyCompleteWithFundingPaysSeller() public {
        ...
        vm.prank(buyer);
        uint256 id =
            board.post(seller, 100e18, EloJobBoard.Mode.ReputationOnly, SPEC, uint64(block.timestamp + 1 days), 0); // no escrow, no premium
        vm.prank(seller);
        board.deliver(id, DELIV);
        vm.startPrank(buyer);
        reserve.approve(address(board), 100e18); // fund at completion time
        board.complete(id);
        vm.stopPrank();
        assertEq(reserve.balanceOf(seller), 1_000e18 + 95e18); // net of the 5% cut
        assertEq(reserve.balanceOf(splitter), 5e18);
        assertEq(reserve.balanceOf(buyer), 1_000e18 - 100e18);
    ...
    function testRepOnlySellerDefaultNothingLockedSlashesSeller() public {
        ...
        vm.warp(block.timestamp + 2 days);
        board.close(id);
        assertEq(reserve.balanceOf(buyer), 1_000e18); // no refund needed: no lock
        assertEq(reserve.balanceOf(seller), 1_000e18);
        assertEq(reputation.rep(seller), 75); // 100 - 25 penalty
    }
    ...
    function testRepOnlyBuyerSilentClosesUnfundedWithHonestZeros() public {
        ...
        emit EloJobBoard.Closed(id, "completed-unfunded", 0, 0); // honest amounts
        board.close(id);
        // NO transfer occurred anywhere:
        assertEq(reserve.balanceOf(seller), 1_000e18);
        assertEq(reserve.balanceOf(buyer), 1_000e18);
    ...
    function testInsuredThinPoolCoverEmitsActualPaidAmount() public {
        reputation.setAuthority(address(this), true);
        reputation.earn(seller, 100, "seed");
        reserve.mint(address(this), 10e18);
        reserve.approve(address(pool), 10e18);
        pool.payPremium(10e18); // the pool holds only 10 of the 95 owed
        vm.startPrank(buyer);
        reserve.approve(address(board), 3e18); // premium only; amount never funded
        uint256 id = board.post(seller, 100e18, EloJobBoard.Mode.Insured, SPEC, uint64(block.timestamp + 1 days), 3e18);
        vm.stopPrank();
        vm.prank(seller);
        board.deliver(id, DELIV);
        vm.warp(block.timestamp + 2 days);
        vm.expectEmit(true, false, false, true, address(board));
        emit EloJobBoard.Closed(id, "completed-insured", 13e18, 0); // 10 + the 3 premium in the pool
        board.close(id);
        assertEq(reserve.balanceOf(seller), 1_000e18 + 13e18); // paid exactly what the pool held
    }
    ...
    function testCloseDeadlineBoundary() public {
        ...
        assertEq(reserve.balanceOf(buyer), 1_000e18); // refunded, never delivered
    }
    ...
    //    a multisig, or a governor. Same shape as EloReputation /
    //    EloInsurancePool / EloHarvest / EloFeeSplitter. ───────────────
    ...
    function testFeeBpsChangeMidJobAppliesAtSettlement() public {
        ...
        vm.prank(buyer);
        board.complete(id);
        assertEq(reserve.balanceOf(seller), 1_000e18 + 90e18); // NEW fee applied
        assertEq(reserve.balanceOf(splitter), 10e18);
    }
    ...
    function testSetArbiterTakesEffect() public {
        ...
        board.setArbiter(IEloArbiter(address(newArb)));
        assertEq(address(board.arbiter()), address(newArb));
        uint256 id = _postEscrow(100e18);
        vm.startPrank(buyer);
        reserve.approve(address(board), 4e18);
        board.dispute(id); // flat fee flows to the NEW arbiter's sink
        vm.stopPrank();
        assertEq(reserve.balanceOf(newSink), 4e18);
        assertEq(reserve.balanceOf(arbFeeSink), 0);
    }
    ...
    function testSetArbiterNonOperatorReverts() public {
        ...
        vm.prank(buyer);
        vm.expectRevert(bytes("not operator"));
        board.setArbiter(IEloArbiter(newArbAddr));
    }
    ...
    function testSetFeeSplitterTakesEffect() public {
        ...
        vm.prank(buyer);
        board.complete(id);
        assertEq(reserve.balanceOf(newSplit), 5e18); // cut lands at the new sink
        assertEq(reserve.balanceOf(splitter), 0);
    }
    ...
    function testPoolClaimCappedAtMaxPayout() public {
        ...
        reserve.mint(address(this), 5_000e18);
        reserve.approve(address(pool), 5_000e18);
        ...
        assertEq(reserve.balanceOf(seller), 1_000e18 + 1_000e18);
    ...
    function testPoolThinPoolPaysWhatItHas() public {
        ...
        assertEq(reserve.balanceOf(seller), 1_000e18 + 10e18);
    ...
    function testFalseReturnEscrowPostUnfundedReverts() public {
        ...
        EloInsurancePool fpool = new EloInsurancePool(IERC20(address(fscry)), 1_000e18);
        EloJobBoard fboard = new EloJobBoard(
        ...
        fboard.post(seller, 100e18, EloJobBoard.Mode.Escrow, SPEC, uint64(block.timestamp + 1 days), 0);
        vm.stopPrank();
    }
    ...
    function testFalseReturnInsuredBuyerDefaultCoveredByPool() public {
        ...
        uint256 id =
            fboard.post(seller, 100e18, EloJobBoard.Mode.Insured, SPEC, uint64(block.timestamp + 1 days), 3e18);
        vm.stopPrank();
        vm.prank(seller);
    ...
    function testCloseAfterDisputeReverts() public {
        uint256 id = _postEscrow(100e18);
        vm.startPrank(buyer);
        reserve.approve(address(board), 2e18); // the flat arbiter fee
    ...
    /// EloReputation.setThreshold — the gate RepOnly mode reads. Effect
    ...
    /// EloReputation.setAuthority — who may mint/slash rep. The effect path
    ...
    /// EloReputation.transferOperator — hand-off correctness end to end.
    ...
    /// EloInsurancePool.setMaxPayout — the per-claim cap. Effect (a claim
    ...
    /// EloInsurancePool.setClaimant — who may drain the pool. Non-operator
    ...
    /// EloInsurancePool.transferOperator — hand-off + guards.
