...
import "../src/ScryEidolon.sol";
import "../src/ScryTicket.sol";
...
contract ScryEidolonTest is Test {
    MockSCRY scry;
    ScryTicket tick;
    ScryEidolon eid;
    ...
    function setUp() public {
        ...
        tick = new ScryTicket(1000, 0, 0, 2 hours, proceeds, 0, "https://scry.moreright.xyz/api");
        eid = new ScryEidolon(
            IERC20(address(scry)), IScryTicket(address(tick)), splitter, 0, commitment, 500,
    ...
    function test_priceZeroPullsNoScry() public {
        uint256 id = _giveTickets(alice, 1);
    ...
    function test_pricedMintStillPullsScryToTheSplitter() public {
        uint256 price = 25_000e18;
        ScryTicket t2 = new ScryTicket(1000, 0, 0, 2 hours, proceeds, 0, "x");
        ScryEidolon priced = new ScryEidolon(
            IERC20(address(scry)), IScryTicket(address(t2)), splitter, price, commitment, 500, "x"
        );
        ...
        vm.prank(alice);
        scry.approve(address(priced), price);
        vm.prank(alice);
        priced.mint(id);
        assertEq(scry.balanceOf(splitter), price);
    ...
    function test_constructorRefusesZeroCommitment() public {
        ...
        new ScryEidolon(IERC20(address(scry)), IScryTicket(address(tick)), splitter, 0, bytes32(0), 500, "x");
    }

    function test_constructorRefusesFatRoyalty() public {
        vm.expectRevert(bytes("royalty > 10%"));
        new ScryEidolon(IERC20(address(scry)), IScryTicket(address(tick)), splitter, 0, commitment, 1001, "x");
    }

    function test_constructorRefusesZeroTicket() public {
        vm.expectRevert(bytes("zero ticket"));
        new ScryEidolon(IERC20(address(scry)), IScryTicket(address(0)), splitter, 0, commitment, 500, "x");
    }
    ...
    function test_constructorRefusesMismatchedTicketSupply() public {
        ...
        new ScryEidolon(IERC20(address(scry)), IScryTicket(address(wrong)), splitter, 0, commitment, 500, "x");
    }
    ...
    function test_tokenURIAndContractURI() public {
        ...
        vm.prank(alice);
        eid.mint(id);
        assertEq(eid.tokenURI(1), "https://scry.moreright.xyz/api/eidolon/1/metadata");
        assertEq(eid.contractURI(), "https://scry.moreright.xyz/api/eidolon/collection");
    ...
    function test_ownerCanRepointBaseURI() public {
        ...
        eid.setBaseURI("https://ipfs.example/scry");
        assertEq(eid.tokenURI(1), "https://ipfs.example/scry/eidolon/1/metadata");
        assertEq(eid.contractURI(), "https://ipfs.example/scry/eidolon/collection");
    }
