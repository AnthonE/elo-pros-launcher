...
contract EloGachaTest is Test {
    ...
    function _statusOf(uint256 id) internal view returns (EloGacha.PositionStatus s) {
        (,,,,,,,,,,,, s,) = g.positions(id);
    }
    ...
    function test_openPoolGatesTheFloorTheBidAndTheSurcharge() public {
        ...
        vm.expectRevert(EloGacha.BelowFloor.selector);
        g.openPool(address(other), CHEAP - 1, 1_000, 8_500);
        vm.expectRevert(EloGacha.BidOutOfRange.selector);
        g.openPool(address(other), CHEAP, 1_000, 9_501);
        vm.expectRevert(EloGacha.BidOutOfRange.selector);
        g.openPool(address(other), CHEAP, 1_000, 4_999);
        vm.expectRevert(EloGacha.SurchargeTooHigh.selector);
        ...
        vm.expectRevert(EloGacha.PoolExists.selector);
        g.openPool(address(nft), CHEAP, 1_000, 8_500);
    }
    ...
    function test_curatorMayOpenPoolsAndBlockingIsSticky() public {
        GachaNft other = new GachaNft();
        vm.prank(stranger);
        vm.expectRevert(EloGacha.OwnerOrCurator.selector);
    ...
    function test_depositBelowTheFloorIsRefused() public {
        vm.prank(alice);
        vm.expectRevert(EloGacha.BelowMinBacking.selector);
    ...
    function test_aCollectionThatLiesAboutTransferIsCaught() public {
        ...
        vm.prank(alice);
        vm.expectRevert(EloGacha.NftTransferFailed.selector);
    ...
    function test_settlePicksThePositionTheSeedPointsAt() public {
        ...
        vm.expectRevert(EloGacha.TooEarly.selector);
        g.settle(drawId);
        ...
        assertEq(uint256(_statusOf(got)), uint256(EloGacha.PositionStatus.Drawn));
    ...
    function test_keepNftMovesTheTokenAndReturnsTheBackingLessTheCut() public {
        ...
        assertEq(uint256(_statusOf(1)), uint256(EloGacha.PositionStatus.Settled));
    ...
    function test_autoBidAboveResolvesInTheSameTransaction() public {
        ...
        _revealHitting(drawId, rich);
        g.settle(drawId);
        assertEq(uint256(_statusOf(rich)), uint256(EloGacha.PositionStatus.Settled), "not resolved atomically");
    ...
    function test_keepNftIsStrictSoTheBuyerCanFallBackToTheBid() public {
        ...
        nft.setPaused(false);
        vm.prank(stranger);
        vm.expectRevert(EloGacha.NotTheRecipient.selector);
        g.recoverStuckNft(1);
        vm.prank(alice);
        g.recoverStuckNft(1);
        assertEq(nft.ownerOf(1), alice);
    ...
    function test_anExpiredDrawForfeitsTheFeeAndNeverRefundsTheBuyer() public {
        ...
        vm.expectRevert(EloGacha.RevealExpired.selector);
        g.settle(drawId);
    ...
    function test_expiryIsTheEscapeThatUnlocksAJammedPool() public {
        ...
        vm.prank(alice);
        vm.expectRevert(EloGacha.DrawPending.selector);
        ...
        vm.expectRevert(EloGacha.StillSettleable.selector);
        g.expire(drawId);
    ...
    function test_withdrawReturnsTheTokenTheBackingAndTheFees() public {
        ...
        assertEq(uint256(_statusOf(2)), uint256(EloGacha.PositionStatus.Withdrawn));
        assertEq(_count(), 0);
    ...
    function test_everyWeiOfAFeeBecomesANamedLiability() public {
        ...
        // a dropped remainder would hide: EloGardener and EloSilo both lost
    ...
    function test_knobBoundsHold() public {
        vm.expectRevert(EloGacha.HouseCutTooHigh.selector);
        g.setHouseBps(501, 100);
        vm.expectRevert(EloGacha.TopShareTooHigh.selector);
        g.setTopKnobs(2_001, 1_000);
        vm.expectRevert(EloGacha.ChoiceExceedsFinalize.selector);
        g.setWindows(2 days, 1 days);
        vm.expectRevert(EloGacha.FinalizeTooLong.selector);
    ...
    function test_theDeployBoundsTheDrawDelay() public {
        vm.expectRevert(EloGacha.DelayOutOfRange.selector);
        new EloGacha(1, sink);
        vm.expectRevert(EloGacha.DelayOutOfRange.selector);
        new EloGacha(129, sink);
        vm.expectRevert(EloGacha.ZeroSink.selector);
        new EloGacha(100, address(0));
    }
}
