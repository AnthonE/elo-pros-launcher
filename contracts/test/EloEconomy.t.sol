...
contract EloBankTest is Test {
    ...
    function setUp() public {
        ...
        bank = new EloBank(IERC20(address(reserve)));
        reserve.mint(alice, 1_000_000e18);
        reserve.mint(bob, 1_000_000e18);
        reserve.mint(games, 1_000_000e18);
        vm.prank(alice);
        reserve.approve(address(bank), type(uint256).max);
        vm.prank(bob);
        reserve.approve(address(bank), type(uint256).max);
    }
    ...
    function test_feeInflowRaisesRedemption() public {
        ...
        vm.prank(games);
        reserve.transfer(address(bank), 500e18);
    ...
    function test_proRataBetweenStakers() public {
        ...
        vm.prank(games);
        reserve.transfer(address(bank), 200e18);
        vm.prank(bob);
    ...
    function test_leaveConservation() public {
        ...
        vm.prank(games);
        reserve.transfer(address(bank), 500e18); // service fees swell the pool
        uint256 poolBefore = reserve.balanceOf(address(bank));
        ...
        assertEq(reserve.balanceOf(alice) - aliceBefore, out); // user-received
        assertEq(poolBefore - reserve.balanceOf(address(bank)), out); // bank delta
    ...
    function test_donationAttackIsUneconomic() public {
        ...
        reserve.mint(attacker, 101e18);
        vm.startPrank(attacker);
        reserve.approve(address(bank), type(uint256).max);
        bank.enter(1001); // attacker keeps 1 share; 1000 locked to the bank
        reserve.transfer(address(bank), 100e18); // donate: 1 share ≈ 0.1 SCRY now
        vm.stopPrank();
        ...
        assertEq(reserve.balanceOf(bob) - bobBefore, victimOut);
    ...
    function test_donationCannotMintZeroSharesToVictim() public {
        ...
        address attacker = address(0xA77AC2);
        reserve.mint(attacker, 1_000_001e18);
        vm.startPrank(attacker);
        reserve.approve(address(bank), type(uint256).max);
        bank.enter(1001);
        reserve.transfer(address(bank), 1_000_000e18); // 100e18*1001/1e24 → 0
        vm.stopPrank();
        vm.prank(bob);
    ...
    function test_leaveTinySharesNeverPaysZero() public {
        ...
        bank.enter(1000e18);
        vm.prank(games);
        reserve.transfer(address(bank), 500_000e18); // 501 SCRY-wei per share-wei
        uint256 bankBefore = reserve.balanceOf(address(bank));
        vm.prank(alice);
        uint256 out = bank.leave(1); // the tiniest possible burn
        assertEq(out, 501); // floor(1 * 501000e18 / 1000e18) — nonzero
        assertEq(bankBefore - reserve.balanceOf(address(bank)), 501);
    ...
    function test_seedIntoAnEmptyBankIsCapturedWholeByTheFirstDepositor() public {
        ...
        reserve.mint(games, seed);
        vm.prank(games);
        reserve.transfer(address(bank), seed); // the §P9 shape, into an empty bank
    ...
    function test_seedAfterAStakeAccruesProRataInsteadOfBeingCaptured() public {
        ...
        reserve.mint(games, seed);
        vm.prank(games);
        reserve.transfer(address(bank), seed); // rate is now ~10 SCRY per xSCRY
...
contract EloFeeSplitterTest is Test {
    MockSCRY reserve;
    EloFeeSplitter split;
    ...
    function setUp() public {
        ...
        to[2] = ops;
        bps[2] = 2000;
        split = new EloFeeSplitter(IERC20(address(reserve)), to, bps);
        reserve.mint(payer, 1_000_000e18);
    }

    function test_distributeSplitsByBps() public {
        vm.prank(payer);
        reserve.transfer(address(split), 1000e18);
        split.distribute();
        assertEq(reserve.balanceOf(bankSink), 500e18);
        assertEq(reserve.balanceOf(prizes), 300e18);
        assertEq(reserve.balanceOf(ops), 200e18);
        assertEq(reserve.balanceOf(address(split)), 0);
    }

    // ── native value: the ERC-2981 path (2026-07-28) ────────────────────────

    /// This contract is `EloEidolon`'s royalty receiver, welded IMMUTABLE from
    ...
    /// this and refuses to broadcast; `DeployEloEidolon` has no such probe.
    ...
    function test_nativeIsNotSweptIntoTheEloSplit() public {
        vm.deal(address(split), 3 ether);
        vm.prank(payer);
        reserve.transfer(address(split), 1000e18);
        split.distribute();
        assertEq(address(split).balance, 3 ether, "distribute() never touches native");
        assertEq(reserve.balanceOf(bankSink), 500e18, "and the SCRY split is unchanged");
    }
    ...
    function test_dustGoesToLastRecipient() public {
        vm.prank(payer);
        reserve.transfer(address(split), 7); // indivisible
        split.distribute();
        assertEq(reserve.balanceOf(address(split)), 0); // nothing sticks
        assertEq(reserve.balanceOf(bankSink) + reserve.balanceOf(prizes) + reserve.balanceOf(ops), 7);
    }
    ...
    function test_burnCutSendsToDeadAndAccounts() public {
        EloFeeSplitter b = _burnSplit(5000, 5000); // 50% burn / 50% ops
        assertEq(b.burnBps(), 5000, "advertised rate");
        assertEq(b.totalBurned(), 0, "nothing burned before a distribute");

        vm.prank(payer);
        reserve.transfer(address(b), 1000e18);
        vm.expectEmit(true, true, true, true);
        emit EloFeeSplitter.Burned(500e18, 500e18);
        b.distribute();

        assertEq(reserve.balanceOf(b.BURN()), 500e18, "the coin is at 0xdEaD");
        ...
        assertEq(reserve.balanceOf(b.BURN()), 1000e18);
    }
    ...
    function test_burnBpsTracksTheSplitAndCannotGoStale() public {
        EloFeeSplitter b = _burnSplit(5000, 5000);
        ...
        reserve.transfer(address(b), 1000e18);
        b.distribute();
        assertEq(reserve.balanceOf(b.BURN()), 0, "and nothing more is burned");
    ...
    function test_twoBurnLinesSumRatherThanOverwrite() public {
        ...
        to[2] = RESERVE_BURN;
        bps[2] = 3000;
        EloFeeSplitter b = new EloFeeSplitter(IERC20(address(reserve)), to, bps);
        ...
        assertEq(reserve.balanceOf(b.BURN()), 600e18);
    }
    ...
    function test_burnAsLastRecipientTakesDustAndAccountsIt() public {
        ...
        vm.prank(payer);
        reserve.transfer(address(b), 7); // indivisible
        b.distribute();
        assertEq(reserve.balanceOf(address(b)), 0, "nothing sticks");
        assertEq(reserve.balanceOf(b.BURN()), 4, "burn took the dust");
    ...
    function test_rescueBypassesTheBurnAndThatIsTheDisclosedTruth() public {
        EloFeeSplitter b = _burnSplit(10000, 0); // 100% burn, on paper
        assertEq(b.burnBps(), 10000);

        vm.prank(payer);
        reserve.transfer(address(b), 1000e18);
        b.rescue(IERC20(address(reserve)), ops, 0); // operator pulls it instead

        assertEq(reserve.balanceOf(b.BURN()), 0, "a 100% burn split burned nothing");
        assertEq(b.totalBurned(), 0, "and the counter does not pretend it did");
        assertEq(reserve.balanceOf(ops), 1000e18);
    }

    /// bps 0 for ops => one-line split; helper keeps the cases readable.
    function _burnSplit(uint16 burnBpsIn, uint16 opsBps) internal returns (EloFeeSplitter) {
        ...
        return new EloFeeSplitter(IERC20(address(reserve)), to, bps);
    }
    ...
    function test_badBpsSumReverts() public {
        ...
        new EloFeeSplitter(IERC20(address(reserve)), to, bps);
    }
    ...
    function test_setSplitDistributesUnderOldRulesFirst() public {
        ...
        vm.prank(payer);
        reserve.transfer(address(split), 100e18);
        split.distribute();
        assertEq(reserve.balanceOf(ops), 200e18 + 100e18);
    }
    ...
    function test_duplicateRecipientMergesAndOverstatesTheOutletCount() public {
        ...
        bps[2] = 2000;
        EloFeeSplitter d = new EloFeeSplitter(IERC20(address(reserve)), to, bps);
        assertEq(d.splitLength(), 3, "the table posts three outlets");

        vm.prank(payer);
        reserve.transfer(address(d), 1000e18);
        d.distribute();
        assertEq(reserve.balanceOf(RESERVE_BURN), 500e18);
        // ...but only TWO addresses were ever paid, and one took both cuts
        assertEq(reserve.balanceOf(ops), 500e18);
    }
    ...
    function test_zeroBpsRecipientAllowedGetsNothing() public {
        ...
        to[2] = ops;
        bps[2] = 5000;
        EloFeeSplitter zsplit = new EloFeeSplitter(IERC20(address(reserve)), to, bps);
        assertEq(zsplit.splitLength(), 3);
        vm.prank(payer);
        reserve.transfer(address(zsplit), 1000e18);
        zsplit.distribute();
        assertEq(reserve.balanceOf(bankSink), 500e18);
        assertEq(reserve.balanceOf(parked), 0); // zero-bps slot receives nothing
        assertEq(reserve.balanceOf(ops), 500e18);
        assertEq(reserve.balanceOf(address(zsplit)), 0);
    }
    ...
    function test_rescue_drains_bypassing_split_and_guards() public {
        address parked = address(0x9A2CED);
        reserve.mint(address(split), 1_000e18);
        vm.prank(payer); // non-operator refused
        vm.expectRevert(bytes("not operator"));
        split.rescue(IERC20(address(reserve)), parked, 1);
        vm.expectRevert(bytes("zero to")); // zero recipient refused
        split.rescue(IERC20(address(reserve)), address(0), 1);
        // partial rescue: exact amount straight out, split untouched
        uint256 got = split.rescue(IERC20(address(reserve)), parked, 400e18);
        assertEq(got, 400e18);
        assertEq(reserve.balanceOf(parked), 400e18);
        assertEq(reserve.balanceOf(address(split)), 600e18);
        // amount == 0 is documented "drain the full balance" (bypassing the
        // split entirely) — pin that it empties the contract to the wei.
        uint256 all = split.rescue(IERC20(address(reserve)), parked, 0);
        assertEq(all, 600e18);
        assertEq(reserve.balanceOf(parked), 1_000e18);
        assertEq(reserve.balanceOf(address(split)), 0);
        // and with nothing left, the zero-amount path has nothing to pull
        vm.expectRevert(bytes("nothing to rescue"));
        split.rescue(IERC20(address(reserve)), parked, 0);
    }
    ...
    function test_distribute_all_or_nothing_when_a_recipient_fails() public {
        ...
        to[1] = bad;
        bps[1] = 5000;
        EloFeeSplitter s = new EloFeeSplitter(IERC20(address(c)), to, bps);
        c.setCursed(bad);
    ...
    function test_feeSplitter_transferOperator() public {
        ...
        split.rescue(IERC20(address(reserve)), payer, 1);
        address[] memory to = new address[](1);
        uint16[] memory bps = new uint16[](1);
        to[0] = payer;
        bps[0] = 10000;
    ...
    /// `EloEidolon.feeSplitter` is IMMUTABLE, so if this ever stops fitting,
...
contract EloHarvestTest is Test {
    MockSCRY reserve;
    EloHarvest harvest;
    ...
    function setUp() public {
        reserve = new MockSCRY();
        harvest = new EloHarvest(IERC20(address(reserve)));
        reserve.mint(address(harvest), 10_000e18);
    }
    ...
    function test_claimAndReclaimCumulative() public {
        ...
        assertEq(got, 40e18);
        assertEq(reserve.balanceOf(w1), 40e18);
        ...
        assertEq(got, 30e18);
        assertEq(reserve.balanceOf(w1), 70e18);
    }
    ...
    function test_claimForSomeoneElsePaysThem() public {
        ...
        harvest.claim(w1, 40e18, proof);
        assertEq(reserve.balanceOf(w1), 40e18);
        assertEq(reserve.balanceOf(w2), 0);
    }
    ...
    function test_sweepNeverBreaksPostedRoot() public {
        ...
        assertEq(reserve.balanceOf(address(this)), 9_900e18);
    }
    ...
    function test_harvest_sweep_gates_and_effect() public {
        ...
        assertEq(reserve.balanceOf(parked), 4_000e18);
        assertEq(reserve.balanceOf(address(harvest)), 6_000e18);
    }
    ...
    function test_harvest_transferOperator() public {
        ...
        assertEq(reserve.balanceOf(w1), 1e18);
    }
    ...
    function test_python_claim_plan_merkle_verifies_onchain() public {
        ...
        assertEq(reserve.balanceOf(wallet), amount);
