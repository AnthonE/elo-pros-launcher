...
import "../src/EloTill.sol";
import "../src/IERC20.sol";
import {MockToken} from "./MockToken.sol";

/// EloTill — the game claim (`EloTill.sol`'s header is the design).
///
...
contract EloTillTest is Test {
    ...
    function setUp() public {
        ...
        till = new EloTill(IERC20(address(tok)), signer, CAP);
    ...
    // ── helpers ─────────────────────────────────────────────────────────────
    function _draft(address who, uint256 cumulative) internal view returns (EloTill.Draft memory d) {
        d = EloTill.Draft({
            wallet: who,
    ...
    function _sign(EloTill.Draft memory d, uint256 pk) internal view returns (bytes memory) {
    ...
    function _digest(EloTill.Draft memory d) internal view returns (bytes32) {
    ...
    function _pay(address who, uint256 cumulative) internal returns (uint256) {
        EloTill.Draft memory d = _draft(who, cumulative);
    ...
    function test_anyone_may_submit_for_anyone_and_the_submitter_is_never_paid() public {
        EloTill.Draft memory d = _draft(alice, 250e18);
    ...
    function test_replaying_the_same_draft_pays_nothing() public {
        EloTill.Draft memory d = _draft(alice, 100e18);
        bytes memory sig = _sign(d, SIGNER_PK);
        till.withdraw(d, sig);
    ...
    function test_out_of_order_drafts_need_no_ordering() public {
        EloTill.Draft memory small = _draft(alice, 100e18);
        EloTill.Draft memory big = _draft(alice, 300e18);
    ...
    function test_a_draft_for_another_instance_does_not_verify_here() public {
        EloTill other = new EloTill(IERC20(address(tok)), signer, CAP);
    ...
    function test_the_cap_is_shared_across_wallets_and_refuses_when_spent() public {
        _pay(alice, CAP);
        EloTill.Draft memory d = _draft(bob, 10e18);
    ...
    /// every twelve days while claiming to be daily. `EloGacha` shipped that
    ...
    function test_an_underfunded_till_reverts_rather_than_paying_less() public {
        ...
        EloTill.Draft memory d = _draft(alice, 10e18);
        vm.expectRevert();
        till.withdraw(d, _sign(d, SIGNER_PK));
    ...
    function testFuzz_never_pays_more_than_the_highest_cumulative(uint96[8] calldata amounts) public {
        ...
        for (uint256 i = 0; i < amounts.length; i++) {
            ...
            if (c == 0) continue;
            if (c > high) high = c;
            EloTill.Draft memory d = _draft(alice, c);
            bytes memory sig = _sign(d, SIGNER_PK);
