...
import "../src/EloFeeConverter.sol";
...
contract EloFeeConverterTest is Test {
    EloFeeConverter conv;
    MockToken reserve;
    MockToken coin;
    ...
    function setUp() public {
        ...
        conv = new EloFeeConverter(
            IERC20(address(reserve)), IERC20(address(coin)), ISwapRouter(address(router)), 10000, payee, steward
        );
    }
    ...
    function test_the_coin_leaves_only_as_a_swap_and_scry_only_to_the_payee() public {
        ...
        assertEq(reserve.balanceOf(address(conv)), 100e18, "and landed back here, not with the steward");
        ...
        assertEq(reserve.balanceOf(payee), 100e18);
    ...
    function test_scry_that_arrives_directly_needs_no_swap() public {
        ...
        reserve.mint(address(conv), 42e18);
        conv.pay();
        assertEq(reserve.balanceOf(payee), 42e18);
    }

    function test_renouncing_leaves_pay_working_forever() public {
        reserve.mint(address(conv), 5e18);
        vm.prank(steward);
        ...
        vm.prank(stranger);
        conv.pay();
        assertEq(reserve.balanceOf(payee), 5e18, "the studio still gets what was already converted");
    }
    ...
    function test_it_refuses_to_be_built_with_scry_as_the_coin() public {
        vm.expectRevert(bytes("reserve cannot be the coin"));
        new EloFeeConverter(
            IERC20(address(reserve)), IERC20(address(reserve)), ISwapRouter(address(router)), 10000, payee, steward
        );
    }
}
