...
/// Stands in for `EloSeatArt`. It ECHOES its arguments rather than drawing
...
contract EloSeatTest is Test {
    ...
    function _deploy(uint16 burnBps) internal returns (EloSeat s) {
    ...
    function test_paid_door_legs_open_independently() public {
        ...
        reserve.mint(alice, 1000e18);
        vm.startPrank(alice);
        reserve.approve(address(seat), 1000e18);
        seat.buyWithReserve();
        vm.stopPrank();
        assertEq(reserve.balanceOf(splitter), 1000e18, "reserve mint proceeds go to the splitter");
    }
    ...
    function test_activation_splits_exactly_at_the_welded_bps() public {
        ...
        reserve.mint(alice, costs[0]);
        vm.startPrank(alice);
        reserve.approve(address(seat), costs[0]);
        seat.activate(id, 1);
        vm.stopPrank();

        assertEq(seat.tierOf(id), 1);
        assertEq(seat.weightOf(id), 100);
        assertEq(reserve.balanceOf(seat.BURN()), costs[0] / 2, "half burns");
        assertEq(reserve.balanceOf(splitter), costs[0] / 2, "half to the splitter");
        assertEq(seat.totalSentToBurn(), costs[0] / 2);
        assertEq(reserve.balanceOf(address(seat)), 0, "nothing sticks to the contract");
    }

    function test_upgrading_pays_only_the_difference() public {
        uint256 id = _mintOneTo(alice);
        (uint256[] memory costs,) = _ladder();
        reserve.mint(alice, costs[4]);
        vm.startPrank(alice);
        reserve.approve(address(seat), costs[4]);
        seat.activate(id, 1);
        seat.activate(id, 4);
        vm.stopPrank();
        ...
        assertEq(reserve.balanceOf(alice), costs[4] - costs[3], "never charged twice for ground held");
    }

    function test_cannot_downgrade_or_re_pay_a_tier() public {
        uint256 id = _mintOneTo(alice);
        (uint256[] memory costs,) = _ladder();
        reserve.mint(alice, costs[4] * 2);
        vm.startPrank(alice);
        reserve.approve(address(seat), costs[4] * 2);
        seat.activate(id, 3);
    ...
    function test_a_zero_burn_deploy_sends_everything_to_the_splitter() public {
        ...
        reserve.mint(alice, costs[0]);
        vm.startPrank(alice);
        reserve.approve(address(s), costs[0]);
        s.activate(id, 1);
        vm.stopPrank();
        assertEq(s.totalSentToBurn(), 0);
        assertEq(reserve.balanceOf(splitter), costs[0]);
    }
    ...
    function test_the_clear_covers_every_transfer_path() public {
        ...
        uint256 id = _mintOneTo(alice);
        reserve.mint(alice, costs[0] * 3);
        vm.startPrank(alice);
        reserve.approve(address(seat), costs[0] * 3);
        seat.activate(id, 1);
    ...
    function test_token_uri_shape() public {
        uint256 id = _mintOneTo(alice);
        assertEq(seat.tokenURI(id), "https://elopros.com/api/seat/1/metadata");
        assertEq(seat.contractURI(), "https://elopros.com/api/seat/collection");
    ...
    // the baseURI bootstrap. `DeploySeat.s.sol` wires `EloSeatArt` on the same
    ...
    function test_the_renderer_is_handed_the_seats_live_tier_door_and_salt() public {
        ...
        (uint256[] memory costs,) = _ladder();
        reserve.mint(alice, costs[2]);
        vm.startPrank(alice);
        reserve.approve(address(seat), costs[2]);
        seat.activate(id, 3);
        vm.stopPrank();
    ...
    function test_tier_changes_weight_and_nothing_else_on_chain() public {
        ...
        assertEq(seat.tokenURI(id), "https://elopros.com/api/seat/1/metadata", "paying changed no metadata path");
    }
    ...
    function _deployWithWindow(uint256 until_) internal returns (EloSeat s) {
        (uint256[] memory costs, uint32[] memory weights) = _ladder();
        s = new EloSeat(
            "reserve seats",
            "SEAT",
    ...
    function test_the_window_hands_an_unclaimed_reserve_to_the_float() public {
        EloSeat s = _deployWithWindow(block.timestamp + 30 days);
    ...
    function test_the_free_doors_shut_when_the_window_does() public {
        EloSeat s = _deployWithWindow(block.timestamp + 1 days);
