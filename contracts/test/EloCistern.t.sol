...
contract EloCisternTest is Test {
    EloSeat seat;
    EloCistern cistern;
    MockCoin reserve;
    MockCoin obol;
    ...
    function setUp() public {
        ...
        cistern = new EloCistern(
            IERC20(address(reserve)), IEloSeat(address(seat)), ISwapRouter(address(router)), THRESHOLD, 100, WINDOW
        );
    }
    ...
    function _activate(address who, uint256 id, uint8 tier) internal {
        ...
        reserve.mint(who, costs[tier - 1]);
        vm.startPrank(who);
        reserve.approve(address(seat), costs[tier - 1]);
    ...
    function _fill(uint256 amount) internal {
        ...
        reserve.approve(address(cistern), amount);
        cistern.fill(amount);
    }
    ...
    function test_the_bar_fills_from_a_bare_transfer() public {
        ...
        reserve.mint(address(this), 250e18);
        reserve.transfer(address(cistern), 250e18);
    ...
    function test_anyone_opens_and_the_caller_keeps_the_tip() public {
        ...
        assertEq(roundId, 0);
        assertEq(reserve.balanceOf(carol), THRESHOLD / 100, "1% tip to whoever pulled the lever");
        EloCistern.Round memory r = cistern.rounds(0);
    ...
    function test_shares_are_tier_weighted() public {
        ...
        EloCistern.Round memory r = cistern.rounds(0);
        uint256 before_a = reserve.balanceOf(alice);
        uint256 before_b = reserve.balanceOf(bob);
        ...
        uint256 gotA = reserve.balanceOf(alice) - before_a;
        uint256 gotB = reserve.balanceOf(bob) - before_b;
    ...
    function test_no_election_pays_scry_with_no_router_involved() public {
        ...
        assertEq(reserve.balanceOf(alice), share, "paid in SCRY, no swap, no route needed");
    ...
    function test_an_election_buys_the_coins_at_market() public {
        ...
        assertEq(reserve.balanceOf(alice), 0, "nothing came back as SCRY");
        assertEq(reserve.balanceOf(address(cistern)), 0, "nothing stuck to the cistern");
    }
    ...
    function test_a_track_with_no_posted_route_fails_open_to_scry() public {
        ...
        vm.prank(alice);
        uint256 share = cistern.claim(0, a, _zeros());
        assertEq(reserve.balanceOf(alice), share, "a claim is never blocked on an unposted pool");
    }
    ...
    function test_claims_never_exceed_the_round() public {
        ...
        EloCistern.Round memory after_ = cistern.rounds(0);
        assertLe(after_.paid, r.pot);
        assertEq(reserve.balanceOf(address(cistern)), cistern.outstanding(), "books match the balance");
    }
    ...
    function test_there_is_no_rescue_pause_or_admin_withdrawal() public {
        _fill(THRESHOLD);
        uint256 held = reserve.balanceOf(address(cistern));
        ...
        cistern.proposeThreshold(1);
        cistern.proposeRoute(1, address(obol), 10_000);
        assertEq(reserve.balanceOf(address(cistern)), held, "not one wei moved");
    }
    ...
    function test_the_cistern_mints_nothing() public {
        ...
        _activate(alice, a, 1);
        _fill(THRESHOLD);
        uint256 supplyBefore = reserve.balanceOf(address(cistern)) + reserve.balanceOf(alice) + reserve.balanceOf(address(this));
        _tick();
        cistern.open();
        vm.prank(alice);
        cistern.claim(0, a, _zeros());
        uint256 supplyAfter = reserve.balanceOf(address(cistern)) + reserve.balanceOf(alice) + reserve.balanceOf(address(this));
