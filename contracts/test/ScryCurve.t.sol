...
/// is for `ScryLaunchpad` (`LAUNCHPAD.md` §gates).
contract ScryCurveTest is Test {
    ScryCurve curve;
    MockToken coin;
    MockToken scry;
    ...
    function _buy(address who, uint256 amount) internal returns (uint256 out) {
        scry.mint(who, amount);
        vm.startPrank(who);
        scry.approve(address(curve), amount);
    ...
    function test_the_curve_opens_with_no_scry_in_it_at_all() public {
        _fundAndArm();
        assertEq(scry.balanceOf(address(curve)), 0, "no quote is required to open");
    ...
    function test_every_wei_of_the_pools_quote_side_came_from_buyers() public {
        ...
        (address t0,) = address(scry) < address(coin) ? (address(scry), address(coin)) : (address(coin), address(scry));
        uint256 quoteIntoPool = t0 == address(scry) ? npm.totalAmount0() : npm.totalAmount1();
    ...
    function test_a_buyer_can_sell_back_on_the_very_next_block() public {
        ...
        uint256 before = scry.balanceOf(alice);
        uint256 back = _sell(alice, got);
        assertGt(back, 0, "the curve bid for it");
        assertEq(scry.balanceOf(alice), before + back, "and paid in scry");
    }
    ...
    function test_trading_fees_split_by_the_posted_share() public {
        ...
        vm.prank(bob);
        curve.payFees();
        assertEq(scry.balanceOf(protocolFees), (owed * PROTOCOL_SHARE) / 10_000, "the protocol's share");
        assertEq(scry.balanceOf(creatorFees), owed - (owed * PROTOCOL_SHARE) / 10_000, "the creator takes the rest");
    ...
    function test_fees_are_held_apart_from_the_reserve() public {
        ...
        assertEq(scry.balanceOf(address(curve)), 100_000e18, "both sit here, counted separately");
    }
    ...
    function test_pool_fees_go_to_the_two_welded_recipients() public {
        ...
        (address t0,) = address(scry) < address(coin) ? (address(scry), address(coin)) : (address(coin), address(scry));
        (uint256 quoteOwed, uint256 coinOwed) = t0 == address(scry) ? (uint256(5e18), uint256(7e18)) : (uint256(7e18), uint256(5e18));
        assertEq(scry.balanceOf(quotePoolFees), quoteOwed, "the quote leg");
    ...
    function test_a_donation_cannot_move_the_price() public {
        ...
        scry.mint(address(curve), 500_000e18); // a gift, straight to the balance
    ...
    function test_an_unknown_fee_tier_refuses_rather_than_minting_a_bad_range() public {
        ...
        new ScryCurve(
            IERC20(address(coin)), IERC20(address(scry)), ICurvePositionManager(address(npm)), uint24(1234),
            VIRTUAL_QUOTE, CURVE_ALLOC, POOL_ALLOC, THRESHOLD, FEE_BPS, PROTOCOL_SHARE,
            creatorFees, protocolFees, quotePoolFees, coinPoolFees
        );
    }
    ...
    function test_a_fee_a_seller_could_not_survive_is_refused() public {
        ...
        new ScryCurve(
            IERC20(address(coin)), IERC20(address(scry)), ICurvePositionManager(address(npm)), POOL_FEE,
    ...
    function test_the_range_is_full_and_aligned_to_the_tiers_spacing() public view {
        ...
        // and failed against a correct contract. `ScryLaunchpad.t.sol` hit this
    ...
    function test_a_launch_end_to_end() public {
        // the studio funds the coin. nothing mints, and no scry is needed.
        _fundAndArm();
        assertEq(scry.balanceOf(address(curve)), 0);
        ...
        assertEq(scry.balanceOf(address(curve)), curve.feesAccrued() + curve.quoteReserve(), "only fees and dust are left here");
        assertGt(raised, 0);
