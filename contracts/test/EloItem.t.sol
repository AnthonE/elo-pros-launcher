...
/// Stands in for `EloGacha`'s custody path: pull with plain `transferFrom`,
/// assert we hold it, hand it on later. If EloItem ever grows a hook, a pause
/// or a soulbind, this is what breaks first — which is the point of it.
contract GachaLike {
    function pull(EloItem item, address from, uint256 tokenId) external {
        item.transferFrom(from, address(this), tokenId);
        require(item.ownerOf(tokenId) == address(this), "NftTransferFailed");
    }

    function push(EloItem item, address to, uint256 tokenId) external {
        item.transferFrom(address(this), to, tokenId);
    }
}

contract EloItemTest is Test {
    EloItem item;
    ...
    function setUp() public {
        vm.prank(creator);
        item = new EloItem("Gates Items", "GATE", payout, 500, "https://elopros.com");
        vm.prank(creator);
    ...
    function test_base_uri_repoints_art_without_touching_the_reference() public {
        vm.prank(minter);
        uint256 id = item.mint(alice, CAT, 0, _ref(1));
        assertEq(item.tokenURI(id), "https://elopros.com/item/1/metadata");
