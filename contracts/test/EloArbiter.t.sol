...
contract EloArbiterTest is Test {
    EloArbiter arb;
    MockCaseBook cases;
    ...
    event Voted(uint256 indexed jobId, address indexed arbiter, IEloArbiter.Verdict verdict, bytes32 reasonHash);
    event Ruled(
        uint256 indexed jobId, IEloArbiter.Verdict verdict, uint256 nBuyer, uint256 nSeller, uint256 nUndecided
    );
    ...
    function setUp() public {
        ...
        p[1] = a2;
        p[2] = a3;
        arb = new EloArbiter(feeSink, FEE, p, 2); // quorum 2 of 3 (a strict majority)
    ...
    function test_voteRefusesACaseTheBoardHasNotIssued() public {
        ...
        arb.vote(3, IEloArbiter.Verdict.ForSeller, "reason");

        vm.prank(a1);
        vm.expectRevert(bytes("no such case on the bound board"));
        arb.vote(9999, IEloArbiter.Verdict.ForSeller, "reason");

        vm.prank(a1); // the last id that DOES exist is fine
        arb.vote(2, IEloArbiter.Verdict.ForSeller, "reason");
    ...
    function test_aSecondBoardCannotSettleOnThisPanelsVerdicts() public {
        vm.prank(a1);
        arb.vote(4, IEloArbiter.Verdict.ForSeller, "r1");
        vm.prank(a2);
        arb.vote(4, IEloArbiter.Verdict.ForSeller, "r2");
        ...
        vm.prank(stranger);
        (bool isFinal, IEloArbiter.Verdict v) = arb.verdictOf(4);
        assertTrue(isFinal);
        assertEq(uint256(v), uint256(IEloArbiter.Verdict.ForSeller));
    }
    ...
    function _freshPanel() internal returns (EloArbiter) {
        address[] memory p = new address[](3);
        p[0] = a1;
        p[1] = a2;
        p[2] = a3;
        return new EloArbiter(feeSink, FEE, p, 2);
    }
    ...
    function test_constructorRejectsZeroFeeRecipient() public {
        ...
        new EloArbiter(address(0), FEE, p, 1);
    }
    ...
    function test_constructorRejectsDuplicateAndZeroMembers() public {
        ...
        new EloArbiter(feeSink, FEE, dup, 2);
        address[] memory zero = new address[](1);
        zero[0] = address(0);
        vm.expectRevert(bytes("bad panel member"));
        new EloArbiter(feeSink, FEE, zero, 1);
    }

    function test_constructorRejectsEmptyPanelAndNonMajorityQuorum() public {
        address[] memory none = new address[](0);
        vm.expectRevert(bytes("quorum not a majority"));
        new EloArbiter(feeSink, FEE, none, 1);
        address[] memory p = new address[](3);
        p[0] = a1;
        p[1] = a2;
        p[2] = a3;
        vm.expectRevert(bytes("quorum not a majority"));
        new EloArbiter(feeSink, FEE, p, 1); // 1 of 3 is not a majority
        vm.expectRevert(bytes("quorum not a majority"));
        new EloArbiter(feeSink, FEE, p, 4); // > panel
    }
    ...
    function test_constructorAcceptsOneOfOne() public {
        ...
        address[] memory p = new address[](1);
        p[0] = a1;
        EloArbiter solo = new EloArbiter(feeSink, FEE, p, 1);
    ...
    function test_majorityForSellerFinalizesWithEvents() public {
        ...
        emit Voted(1, a1, IEloArbiter.Verdict.ForSeller, bytes32("r1"));
        vm.prank(a1);
        arb.vote(1, IEloArbiter.Verdict.ForSeller, bytes32("r1"));
        ...
        vm.expectEmit(true, true, true, true);
        emit Ruled(1, IEloArbiter.Verdict.ForSeller, 0, 2, 0); // tallies ride the event
        vm.prank(a2);
        arb.vote(1, IEloArbiter.Verdict.ForSeller, bytes32("r2"));
        assertTrue(arb.finalized(1));
        assertEq(arb.forSeller(1), 2);
        (bool fin, IEloArbiter.Verdict v) = arb.verdictOf(1);
        assertTrue(fin);
        assertEq(uint256(v), uint256(IEloArbiter.Verdict.ForSeller));
        vm.prank(address(cases));
        assertEq(uint256(arb.rule(1, bytes32(0), bytes32(0))), uint256(IEloArbiter.Verdict.ForSeller));
    }

    function test_majorityForBuyerFinalizes() public {
        vm.prank(a1);
        arb.vote(7, IEloArbiter.Verdict.ForBuyer, "r1");
        vm.prank(a2);
        arb.vote(7, IEloArbiter.Verdict.ForBuyer, "r2");
        vm.prank(address(cases));
        assertEq(uint256(arb.rule(7, bytes32(0), bytes32(0))), uint256(IEloArbiter.Verdict.ForBuyer));
    }

    // ── a full split with no majority → Undecided (the board refunds) ──
    function test_splitPanelRulesUndecided() public {
        vm.prank(a1);
        arb.vote(3, IEloArbiter.Verdict.ForSeller, "r1");
        vm.prank(a2);
        arb.vote(3, IEloArbiter.Verdict.ForBuyer, "r2");
        assertFalse(arb.finalized(3)); // 1-1, no verdict reached quorum
        vm.prank(a3); // whole bench voted, no majority
        arb.vote(3, IEloArbiter.Verdict.Undecided, "r3");
        assertTrue(arb.finalized(3));
        vm.prank(address(cases));
        assertEq(uint256(arb.rule(3, bytes32(0), bytes32(0))), uint256(IEloArbiter.Verdict.Undecided));
    }

    // ── rule() reverts before the panel has ruled (procedural, not a verdict) ──
    function test_ruleRevertsBeforeQuorum() public {
        vm.prank(a1);
        arb.vote(9, IEloArbiter.Verdict.ForSeller, "r1");
    ...
    function test_nonArbiterCannotVote() public {
        ...
        arb.vote(1, IEloArbiter.Verdict.ForSeller, "x");
    }

    function test_doubleVoteReverts() public {
        vm.prank(a1);
        arb.vote(1, IEloArbiter.Verdict.ForSeller, "r1");
        vm.prank(a1);
        vm.expectRevert(bytes("already voted"));
        arb.vote(1, IEloArbiter.Verdict.ForBuyer, "r1b");
    }
    ...
    function test_voteAfterFinalizedReverts() public {
        ...
        arb.vote(1, IEloArbiter.Verdict.ForSeller, "r2");
        vm.prank(a3);
        vm.expectRevert(bytes("already ruled"));
        arb.vote(1, IEloArbiter.Verdict.ForBuyer, "r3");
    }
    ...
    function test_caseQuorumPinnedAgainstLaterRaise() public {
        ...
        p[3] = a4;
        p[4] = address(0xA5);
        EloArbiter big = new EloArbiter(feeSink, FEE, p, 3);
        big.setBoard(address(cases)); // every panel serves a board before it rules
        vm.prank(a1);
        big.vote(11, IEloArbiter.Verdict.ForSeller, "r1");
        vm.prank(a2);
        big.vote(11, IEloArbiter.Verdict.ForSeller, "r2");
        big.setQuorum(4); // 4 of 5 is a majority; global bar moves
        vm.prank(a3);
        big.vote(11, IEloArbiter.Verdict.ForSeller, "r3");
        assertTrue(big.finalized(11)); // pinned caseQuorum 3 ruled, not the new 4
        vm.prank(address(cases));
        assertEq(uint256(big.rule(11, bytes32(0), bytes32(0))), uint256(IEloArbiter.Verdict.ForSeller));
    }
}

/// The integration the deploy path needs: a REAL EloArbiter wired into a REAL
/// EloJobBoard settles disputes, with the flat fee independent of the verdict
/// across ALL THREE verdicts. This is what DeployEloMarket.s.sol expects
...
contract EloArbiterBoardTest is Test {
    MockSCRY reserve;
    EloReputation reputation;
    ...
    function setUp() public {
        ...
        reserve.mint(seller, 1_000e18);
    }

    function _postEscrow(uint256 amount) internal returns (uint256 id) {
        vm.startPrank(buyer);
        reserve.approve(address(board), amount);
        id = board.post(seller, amount, EloJobBoard.Mode.Escrow, SPEC, uint64(block.timestamp + 1 days), 0);
        vm.stopPrank();
    }

    function _panelRules(uint256 id, IEloArbiter.Verdict v) internal {
        vm.prank(a1);
        arb.vote(id, v, "r1");
    ...
    function _panelSplits(uint256 id) internal {
        ...
        vm.prank(a1);
        arb.vote(id, IEloArbiter.Verdict.ForSeller, "r1");
        vm.prank(a2);
        arb.vote(id, IEloArbiter.Verdict.ForBuyer, "r2");
        vm.prank(a3);
        arb.vote(id, IEloArbiter.Verdict.Undecided, "r3");
    }

    // ── a real panel ruling ForSeller pays the seller ──
    function test_disputeRuledForSellerPays() public {
        uint256 id = _postEscrow(100e18);
        _panelRules(id, IEloArbiter.Verdict.ForSeller);
        vm.startPrank(buyer);
        reserve.approve(address(board), FEE);
        board.dispute(id);
        vm.stopPrank();
        assertEq(reserve.balanceOf(seller), 1_000e18 + 95e18); // 100 - 5% fee
        assertEq(reserve.balanceOf(arbFeeSink), FEE); // the flat fee landed
    }
    ...
    function test_disputeRuledForBuyerRefundsAndSlashes() public {
        ...
        uint256 id = _postEscrow(100e18);
        _panelRules(id, IEloArbiter.Verdict.ForBuyer);
        vm.startPrank(buyer);
        reserve.approve(address(board), FEE);
        board.dispute(id);
        vm.stopPrank();
        assertEq(reserve.balanceOf(buyer), 1_000e18 - FEE); // escrow refunded, minus the arb fee it paid
    ...
    function test_disputeUndecidedRefundsWithoutSlash() public {
        ...
        emit EloJobBoard.Closed(id, "undecided", 0, 0);
        board.dispute(id);
        vm.stopPrank();
        assertEq(reserve.balanceOf(buyer), 1_000e18 - FEE); // escrow refunded
        assertEq(reserve.balanceOf(seller), 1_000e18); // seller paid nothing, got nothing
    ...
    function test_arbiterFeeIndependentOfVerdictOnBoard() public {
        ...
        _panelRules(id1, IEloArbiter.Verdict.ForSeller);
        vm.startPrank(buyer);
        reserve.approve(address(board), FEE);
        board.dispute(id1);
        vm.stopPrank();
        uint256 after1 = reserve.balanceOf(arbFeeSink);

        uint256 id2 = _postEscrow(100e18);
        _panelRules(id2, IEloArbiter.Verdict.ForBuyer);
        vm.startPrank(buyer);
        reserve.approve(address(board), FEE);
        board.dispute(id2);
        vm.stopPrank();
        uint256 after2 = reserve.balanceOf(arbFeeSink);
        ...
        board.dispute(id3);
        vm.stopPrank();
        uint256 after3 = reserve.balanceOf(arbFeeSink);
    ...
    function test_sellerInitiatedDispute() public {
        ...
        vm.stopPrank();
        assertEq(reserve.balanceOf(seller), 1_000e18 + 95e18 - FEE); // paid, minus the fee they fronted
        assertEq(reserve.balanceOf(arbFeeSink), FEE);
    }
    ...
    function test_repOnlyDisputeForSellerUnfundedPaysNothing() public {
        ...
        vm.prank(buyer);
        uint256 id =
            board.post(seller, 100e18, EloJobBoard.Mode.ReputationOnly, SPEC, uint64(block.timestamp + 1 days), 0);
        vm.prank(seller);
        board.deliver(id, DELIV);
        _panelRules(id, IEloArbiter.Verdict.ForSeller);
        vm.startPrank(seller);
        reserve.approve(address(board), FEE);
        vm.expectEmit(true, false, false, true, address(board));
        emit EloJobBoard.Closed(id, "completed-unfunded", 0, 0);
        board.dispute(id);
        vm.stopPrank();
        assertEq(reserve.balanceOf(seller), 1_000e18 - FEE); // won the case; zero SCRY arrived
    ...
    function test_disputeBeforePanelRulesRevertsAndKeepsFee() public {
        ...
        board.dispute(id);
        vm.stopPrank();
        assertEq(reserve.balanceOf(arbFeeSink), 0); // the fee never left
        assertEq(reserve.balanceOf(address(board)), 100e18); // escrow still held, the job still open
    }
