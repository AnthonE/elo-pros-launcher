...
/// a `ScryGarden` never holds canonical SCRY (audit F5), and MYRRH because the
/// Garden is its only source since 2026-07-26 (§3a; era-0 6,000/day). This harness
/// stakes a stand-in `ScryGarden(OBOL, mock-SCRY)` on purpose: the Gardener
...
contract ScryGardenerTest is Test {
    SpoilsToken obol;
    SpoilsToken myrrh;
    ...
    function setUp() public {
        ...
        tscry = new MockToken("stand-in scry", "tSCRY");
        garden = new ScryGarden(IERC20(address(obol)), IERC20(address(tscry)));

        unlockAt = vm.getBlockTimestamp() + 90 days;
        granary = new ScryGranary(myrrh); // steward = this
        gardener = new ScryGardener(granary, RPS, LOCK_BPS, unlockAt); // owner = this
    ...
    function test_constructor_guards() public {
        vm.expectRevert("lock too high");
        new ScryGardener(granary, RPS, 9001, vm.getBlockTimestamp() + 1 days);
        vm.expectRevert("unlock in past");
        new ScryGardener(granary, RPS, 100, vm.getBlockTimestamp());
    }
    ...
    /// Port of ScrySilo's rate-change shape: setRewardPerSecond runs
    ...
    function test_production_rps_per_second_updates_lose_nothing_carry_conserves() public {
        ...
        ScryGardener prod = new ScryGardener(granary, prodRps, 6700, cliff);
    ...
    function test_schedule_lifetime_fits_under_the_myrrh_cap() public {
        ...
        ScryGranary g2 = new ScryGranary(myrrh);
        ScryGardener prod = new ScryGardener(g2, posted, 6700, vm.getBlockTimestamp() + 90 days);
