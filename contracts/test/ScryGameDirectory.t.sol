...
contract ScryGameDirectoryTest is Test {
    ScryGameDirectory dir;
    ScryGameTicketFactory factory;
    MockToken scry;
    MockToken usdg;
    ...
    function setUp() public {
        ...
        ticketCode = type(ScryGameTicket).creationCode;
        factory = new ScryGameTicketFactory(keccak256(ticketCode));
        dir = new ScryGameDirectory(address(this));
    ...
    function _params(string memory game, address owner_)
        internal
        view
        returns (ScryGameTicketFactory.TicketParams memory p)
    {
        p = ScryGameTicketFactory.TicketParams({
            name: "Gates",
            symbol: "GATES-COPY",
            game: game,
            scry: address(scry),
            usdg: address(usdg),
            scrySink: address(splitter),
            proceeds: proceeds,
            supplyCap: 0,
            baseURI: "https://scry.moreright.xyz",
            owner: owner_
        });
    }
    ...
    function _uris() internal pure returns (string[] memory u) {
        u = new string[](2);
        u[0] = "https://scry.moreright.xyz/api/store/catalog";
    ...
    function test_anyone_may_deploy_because_gating_it_would_gate_nothing() public {
        ...
        assertEq(ScryGameTicket(t).owner(), stranger, "the deployer owns it when no owner is named");
    ...
    function test_predict_answers_before_the_address_exists() public {
        ScryGameTicketFactory.TicketParams memory p = _params("gates", address(this));
    ...
    function test_the_ticket_owner_is_set_at_birth_so_no_contract_holds_a_live_sale() public {
        ...
        assertEq(ScryGameTicket(t).owner(), studio);
        assertEq(ScryGameTicket(t).pendingOwner(), address(0), "no second step is owed");
        // the studio can price its own game immediately
        vm.prank(studio);
        ScryGameTicket(t).setPrices(1000, 1 ether, 0, 0);
        assertEq(ScryGameTicket(t).priceWei(), 1 ether);
    }
    ...
    function test_a_keeper_lists_and_a_stranger_walks_the_whole_shelf() public {
        ...
        (string[] memory slugs, ScryGameDirectory.Listing[] memory rows) = dir.page(0, 0);
    ...
    function test_a_hand_deployed_ticket_lists_exactly_like_a_factory_one() public {
        ScryGameTicket hand = new ScryGameTicket(
            "Indie", "INDIE-COPY", "indie", IERC20(address(scry)), IERC20(address(usdg)),
    ...
    function test_an_untrusted_factory_is_refused_even_if_it_attests() public {
        ScryGameTicketFactory rogue = new ScryGameTicketFactory(keccak256(ticketCode));
    ...
    function test_listNew_deploys_and_shelves_in_one_transaction() public {
        ...
        assertEq(ScryGameTicket(t).owner(), studio, "the studio owns its own sale from block one");
        assertTrue(factory.deployedByFactory(t));
    }
    ...
    function test_the_directory_owns_no_ticket_so_it_can_never_touch_a_price() public {
        ...
        assertTrue(ScryGameTicket(t).owner() != address(dir));
        vm.prank(address(dir));
        vm.expectRevert(bytes("not owner"));
        ScryGameTicket(t).setPrices(1000, 1, 0, 0);
    }
    ...
    function test_paging_past_the_end_returns_empty_rather_than_reverting() public {
        _listed();
        (string[] memory slugs, ScryGameDirectory.Listing[] memory rows) = dir.page(5, 10);
