...
import "../src/ScryPeculium.sol";
import "../src/ScryPeculiumFactory.sol";
...
contract ScryPeculiumTest is Test {
    ...
    function setUp() public {
        tok = new MockToken();
        factory = new ScryPeculiumFactory();
    ...
    function test_precomputedAddressIsTheAdmittedVault() public {
        ScryPeculium.Init memory init = _init();
    ...
    function test_minorityVerifierSignaturesCannotDeployVault() public {
        ScryPeculium.Init memory init = _init();
        (ScryPeculium.SignerAdmission memory admission, bytes memory proof, bytes[] memory signatures) =
            _initialBundle(init, signer, signerKey, false, 2);
        vm.expectRevert(ScryPeculium.InvalidSignature.selector);
    ...
    function test_wrongPolicyOrCandidateProofCannotDeployVault() public {
        ...
        vm.expectRevert(ScryPeculium.InvalidAdmission.selector);
        factory.deploy(init, admission, proof, signatures);
    ...
    function test_onlyAdmittedSignerCanSpendAndCapsStillHold() public {
        ...
        vm.prank(signer);
        vm.expectRevert(ScryPeculium.NotAllowed.selector);
        vault.spend(rando, 1 ether);
        vm.prank(signer);
        vm.expectRevert(ScryPeculium.OverPerTxCap.selector);
        vault.spend(venue, PER_TX + 1);
        vm.prank(owner);
        vm.expectRevert(ScryPeculium.NotSigner.selector);
    ...
    function test_windowCapAccumulatesAndResets() public {
        ...
        vault.spend(venue, 100 ether);
        vault.spend(venue, 100 ether);
        vm.expectRevert(ScryPeculium.OverWindowCap.selector);
        vault.spend(venue, 100 ether);
    ...
    function test_ownerAloneCannotAdmitAnEOA() public {
        ...
        (ScryPeculium.SignerAdmission memory admission, bytes memory proof, bytes[] memory noVerifierSignatures) =
    ...
    function test_emergencyRotationFailsClosedUntilHeartbeatDelayThenNeedsQuorum() public {
        ...
        vm.expectRevert(ScryPeculium.EmergencyTooEarly.selector);
        vault.admitSigner(admission, proof, bytes(""), signatures);
    ...
    function _init() internal view returns (ScryPeculium.Init memory init) {
    ...
    function _initialBundle(
        ...
        internal
        view
        returns (ScryPeculium.SignerAdmission memory admission, bytes memory proof, bytes[] memory signatures)
    {
        address predicted = factory.predict(init);
        ...
        admission = ScryPeculium.SignerAdmission({
            vault: predicted,
    ...
    function _admissionDigest(address vaultAddress, ScryPeculium.SignerAdmission memory admission)
        internal
        view
        returns (bytes32)
    {
        bytes32 domain = keccak256(
            abi.encode(DOMAIN_TYPEHASH, keccak256("ScryPeculiumAdmission"), keccak256("1"), block.chainid, vaultAddress)
        );
