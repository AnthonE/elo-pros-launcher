...
/// can fetch is `https://elopros.com/api/ticket/gates/1/metadata`, and
...
contract EloTicketUrlRendererTest is Test {
    ...
    string constant BASE = "https://elopros.com/api";

    EloTicketUrlRenderer r;
    address owner = address(this);
    address alice = address(0xA11CE);

    function setUp() public {
        r = new EloTicketUrlRenderer(owner, BASE, "gates");
    }

    function test_the_urls_are_the_paths_the_meter_serves() public view {
        assertEq(r.tokenURI(1), "https://elopros.com/api/ticket/gates/1/metadata");
        assertEq(r.tokenURI(7), "https://elopros.com/api/ticket/gates/7/metadata");
        assertEq(r.contractURI(), "https://elopros.com/api/ticket/gates/collection");
    }
    ...
    function test_the_id_renders_for_every_shape_of_number() public view {
        assertEq(r.tokenURI(0), "https://elopros.com/api/ticket/gates/0/metadata");
        assertEq(r.tokenURI(10), "https://elopros.com/api/ticket/gates/10/metadata");
        assertEq(r.tokenURI(1000), "https://elopros.com/api/ticket/gates/1000/metadata");
        assertEq(
            r.tokenURI(type(uint256).max),
            "https://elopros.com/api/ticket/gates/115792089237316195423570985008687907853269984665640564039457584007913129639935/metadata"
        );
    }
    ...
    function test_the_base_moves_and_only_the_owner_moves_it() public {
        ...
        vm.prank(alice);
        vm.expectRevert(EloTicketUrlRenderer.NotOwner.selector);
    ...
    function test_an_empty_base_is_refused_at_both_doors() public {
        vm.expectRevert(EloTicketUrlRenderer.EmptyBase.selector);
        r.setBase("");

        vm.expectRevert(EloTicketUrlRenderer.EmptyBase.selector);
        new EloTicketUrlRenderer(owner, "", "gates");
    }
    ...
    function test_ownership_is_two_step() public {
        ...
        vm.expectRevert(EloTicketUrlRenderer.NotPending.selector);
        r.acceptOwnership();
    ...
    function test_a_real_ticket_delegates_and_zero_gives_the_built_in_back() public {
        MockToken reserve = new MockToken("Scry", "SCRY");
        MockToken usdg = new MockToken("USD Glo", "USDG");
        address[] memory to = new address[](1);
        uint16[] memory bps = new uint16[](1);
        to[0] = address(0xB0B);
        bps[0] = 10000;
        EloFeeSplitter split = new EloFeeSplitter(IERC20(address(reserve)), to, bps);

        EloGameTicket t = new EloGameTicket(
            "Gates", "GATES-COPY", "gates", IERC20(address(reserve)), IERC20(address(usdg)),
            address(split), address(0xDE7), 0, "https://elopros.com", address(0)
        );
        t.compMint(alice, 1);
        uint256 id = 1;

        t.setRenderer(address(r));
        assertEq(t.tokenURI(id), "https://elopros.com/api/ticket/gates/1/metadata");
        assertEq(t.contractURI(), "https://elopros.com/api/ticket/gates/collection");
