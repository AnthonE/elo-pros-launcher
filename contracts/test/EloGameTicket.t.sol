...
///   - the half-burn is proven against the REAL EloFeeSplitter at the
...
contract EloGameTicketTest is Test {
    EloGameTicket t;
    MockToken reserve;
    MockToken usdg;
    ...
    function setUp() public {
        ...
        t = new EloGameTicket(
            ...
            address(splitter), proceeds, 0, "https://elopros.com", address(0)
        );
        t.setPrices(1000, PRICE_WEI, PRICE_SCRY, PRICE_USDG);

        vm.deal(buyer, 1 ether);
        reserve.mint(buyer, PRICE_SCRY * 3);
    ...
    function test_scry_buy_moves_the_coin_straight_to_the_splitter() public {
        vm.startPrank(buyer);
        reserve.approve(address(t), PRICE_SCRY);
        uint256 id = t.buyWithSCRY();
        vm.stopPrank();
        assertEq(t.ownerOf(id), buyer);
        assertEq(t.railOf(id), 2);
        assertEq(reserve.balanceOf(address(t)), 0); // the ticket never holds SCRY
        assertEq(reserve.balanceOf(address(splitter)), PRICE_SCRY);
    }
    ...
    function test_half_of_a_scry_buy_burns_under_the_posted_split() public {
        ...
        assertEq(reserve.balanceOf(RESERVE_BURN), PRICE_SCRY / 2);
        assertEq(reserve.balanceOf(bank), PRICE_SCRY * 4000 / 10_000);
        assertEq(reserve.balanceOf(ops), PRICE_SCRY - PRICE_SCRY / 2 - PRICE_SCRY * 4000 / 10_000);
    ...
    function test_supply_cap_binds_every_door_when_set() public {
        EloGameTicket capped = new EloGameTicket(
            "Gates", "GATES", "gates", IERC20(address(reserve)), IERC20(address(usdg)),
            address(splitter), proceeds, 2, "https://elopros.com", address(0)
        );
    ...
    function test_tokenURI_is_a_self_contained_document_needing_no_server() public {
        ...
        assertTrue(_contains(json, '"external_url":"https://elopros.com/game.html?slug=gates"'), json);
    }

    function test_contractURI_is_self_contained_too() public view {
        string memory json = _json(t.contractURI());
        assertTrue(_contains(json, '"name":"Gates on Elo Pros"'), json);
        assertTrue(_contains(json, '"image":"data:image/svg+xml;base64,'), json);
    }
    ...
    function test_the_collection_and_the_token_each_use_their_own_link_key() public {
        ...
        string memory url = "https://elopros.com/game.html?slug=gates";
    ...
    function test_refreshMetadata_pokes_a_marketplace_and_changes_nothing() public {
        ...
        vm.expectEmit(false, false, false, true);
        emit EloGameTicket.BatchMetadataUpdate(1, type(uint256).max);
        t.refreshMetadata();
...
contract EloGameTicketRailsTest is Test {
    EloGameTicket t;
    MockToken reserve;
    MockToken usdg;
    MockToken partner;
    ...
    function setUp() public {
        ...
        t = new EloGameTicket(
            "Gates", "GATES", "gates", IERC20(address(reserve)), IERC20(address(usdg)),
            address(0xF33), proceeds, 0, "https://elopros.com", address(0)
        );
    }
    ...
    function test_the_seeded_rails_are_the_three_every_surface_reads() public view {
        ...
        (uint8 k2, address tok2,,, address s2,, string memory l2) = t.railInfo(t.RAIL_ELO());
        (, address tok3,,, address s3,, string memory l3) = t.railInfo(t.RAIL_USDG());
        assertEq(k1, uint8(EloGameTicket.AssetKind.NATIVE));
        assertEq(l1, "eth");
        assertFalse(o1, "a seeded rail starts SHUT - a deploy posts no price");
        assertEq(k2, uint8(EloGameTicket.AssetKind.ERC20));
        assertEq(tok2, address(reserve));
    ...
    function test_setRail_refuses_the_reserved_ids() public {
        for (uint256 id = 1; id <= 5; id++) {
            ...
            t.setRail(id, EloGameTicket.AssetKind.ERC20, address(partner), 0, 1e18, sink, "nope");
        }
    }

    function test_a_second_native_rail_is_refused() public {
        vm.expectRevert(bytes("native is rail 1"));
        t.setRail(6, EloGameTicket.AssetKind.NATIVE, address(1), 0, 1 ether, sink, "eth2");
    }

    function test_only_the_owner_posts_a_rail() public {
        vm.prank(stranger);
        vm.expectRevert(bytes("not owner"));
        t.setRail(6, EloGameTicket.AssetKind.ERC20, address(partner), 0, 5e18, sink, "ptnr");
    }
    ...
    function test_an_nft_rail_takes_the_collection_and_any_ids_in_it() public {
        t.setRail(7, EloGameTicket.AssetKind.ERC721, address(nft), 0, 2, sink, "stonks");
        nft.mint(buyer, 41);
        nft.mint(buyer, 42);
        vm.startPrank(buyer);
    ...
    function test_the_same_nft_cannot_pay_twice() public {
        t.setRail(7, EloGameTicket.AssetKind.ERC721, address(nft), 0, 2, buyer, "self");
        nft.mint(buyer, 41);
        vm.startPrank(buyer);
        nft.setApprovalForAll(address(t), true);
    ...
    function test_an_erc1155_rail_moves_the_posted_quantity_of_one_id() public {
        t.setRail(8, EloGameTicket.AssetKind.ERC1155, address(multi), 9, 3, sink, "pack");
    ...
    function test_closeRail_shuts_the_scry_leg_and_priceScry_reads_zero() public {
        ...
        assertEq(t.priceReserve(), 700_000e18);
        t.closeRail(t.RAIL_ELO());
        assertEq(t.priceReserve(), 0, "the legacy getter is a VIEW over the rail, so it follows");
    ...
    function test_rails_enumerate_for_a_surface() public {
        ...
        t.setRail(6, EloGameTicket.AssetKind.ERC20, address(partner), 0, 9e18, sink, "ptnr");
...
contract EloGameTicketRaiseTest is Test {
    EloGameTicket t;
    MockToken reserve;
    MockToken usdg;
