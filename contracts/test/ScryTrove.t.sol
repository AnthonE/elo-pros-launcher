...
contract ScryTroveTest is Test {
    ScryTrove trove;
    MockNft skins;
    MockNft art;
    GachaLike gacha;
    ...
    function setUp() public {
        trove = new ScryTrove("https://scry.moreright.xyz/api/trove/");
        skins = new MockNft();
        art = new MockNft();
        gacha = new GachaLike();
    }
    ...
    /// `ScryGacha`'s `mapping(address => Pool)` sees ONE pool holding both.
    ...
    /// `ScryGacha.deposit`'s exact pull, then a buyer unwrapping into the real
    ...
    function test_depositorCannotRecall() public {
        ...
        vm.prank(ARTIST);
        vm.expectRevert(ScryTrove.NotTheOwner.selector);
        trove.unwrap(w);
    }
    ...
    function test_cannotWrapTwice() public {
        ...
        vm.prank(ARTIST);
        vm.expectRevert(ScryTrove.AlreadyWrapped.selector);
        trove.wrap(address(skins), 7);
    }

    function test_notAContract() public {
        vm.prank(HOUSE);
        vm.expectRevert(ScryTrove.NotAContract.selector);
        trove.wrap(HOUSE, 1);
    }
    ...
    function test_rewrapGetsAFreshIdAndTheOldOneStaysBurned() public {
        ...
        assertTrue(w2 > w);
        vm.expectRevert(ScryTrove.NotWrapped.selector);
        trove.ownerOf(w);
    }
    ...
    function test_zeroAmountRefused() public {
        MockToken coin = new MockToken();
        vm.prank(HOUSE);
        vm.expectRevert(ScryTrove.ZeroAmount.selector);
