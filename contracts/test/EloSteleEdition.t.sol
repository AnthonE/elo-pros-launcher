...
import "../src/EloSteleEdition.sol";
...
/// Stands in for the live EloVowRegistry. The print shop asks it one thing:
...
contract EloSteleEditionTest is Test {
    MockSCRY reserve;
    ...
    function setUp() public {
        ...
        ed = new EloSteleEdition(
            IERC20(address(reserve)),
            splitter,
            price,
            "https://elopros.com/api",
            IEloVowRegistryView(address(registry))
        );
    }
    ...
    function test_cannotPrintAVowThatWasNeverSworn() public {
        ...
        vm.prank(alice);
        reserve.approve(address(ed), price);
        vm.prank(alice);
        vm.expectRevert(bytes("no such vow"));
        ed.mintEdition(unswornVowId);
        assertEq(reserve.balanceOf(alice), price, "and not a wei was taken for it");
    ...
    function test_constructorRejectsAZeroRegistry() public {
        ...
        new EloSteleEdition(IERC20(address(reserve)), splitter, price, "b", IEloVowRegistryView(address(0)));
    }

    function test_mintPullsEloToSplitter() public {
        reserve.mint(alice, price);
        vm.prank(alice);
        reserve.approve(address(ed), price);
        vm.prank(alice);
        ...
        assertEq(tokenId, 1);
        assertEq(ed.ownerOf(1), alice);
        assertEq(reserve.balanceOf(splitter), price); // SCRY went straight to the splitter
    ...
    function test_editionsIncrementPerVow() public {
        reserve.mint(alice, price * 3);
        vm.prank(alice);
        reserve.approve(address(ed), price * 3);
        vm.startPrank(alice);
        ed.mintEdition(vowId);
        ed.mintEdition(vowId);
        ...
        assertEq(reserve.balanceOf(splitter), price * 3); // multi-mint conservation: all 3 payments landed
    }
    ...
    function test_safeTransferFrom_receiver_accept_and_reject() public {
        reserve.mint(alice, price * 2);
        vm.prank(alice);
        reserve.approve(address(ed), price * 2);
        vm.prank(alice);
