...
contract EloCovenantTest is Test {
    EloCovenant cov;
    ...
    function test_openEmitsFullText() public {
        ...
        emit EloCovenant.CovenantOpened(cid, momo1, th, 24, "momo fleet - the 5% oath", oath);
        vm.prank(momo1);
        cov.open(cid, th, 24, "momo fleet - the 5% oath", oath);
    ...
    function test_openRejectsBadTextHash() public {
        vm.prank(momo1);
        vm.expectRevert(EloCovenant.BadTextHash.selector);
    ...
    function test_openIsOnce() public {
        _open();
        vm.prank(momo2);
        vm.expectRevert(EloCovenant.CovenantExists.selector);
    ...
    function test_cannotSwearTwice() public {
        ...
        cov.swear(cid);
        vm.prank(momo1);
        vm.expectRevert(EloCovenant.AlreadySworn.selector);
        cov.swear(cid);
    }

    function test_cannotSwearUnopened() public {
        vm.prank(momo1);
        vm.expectRevert(EloCovenant.NoSuchCovenant.selector);
    ...
    function test_renounceIsRecordedNotErased() public {
        ...
        vm.warp(2000);
        vm.expectEmit(true, true, false, true);
        emit EloCovenant.Renounced(cid, momo1, 1, "found a better book", 2000);
        vm.prank(momo1);
    ...
    function test_cannotRenounceTwiceOrReSwear() public {
        ...
        vm.prank(momo1);
        vm.expectRevert(EloCovenant.AlreadyRenounced.selector);
    ...
    function test_cannotRenounceUnsworn() public {
        _open();
        vm.prank(momo2);
        vm.expectRevert(EloCovenant.NotAMember.selector);
    ...
    function test_swearEmitsSworn() public {
        _open();
        vm.expectEmit(true, true, false, true);
        emit EloCovenant.Sworn(cid, momo2, 1, 1000); // _open warped to 1000
        vm.prank(momo2);
        cov.swear(cid);
    }
