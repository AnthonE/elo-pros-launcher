"""elo overlay — the Python reference client. One file, stdlib only.

What a game or an agent uses to reach a running elo launcher: who is playing,
...
that could — see `sdk/PROTOCOL.md` and `docs/client/SDK.md`.
    ...
    from elo_overlay import Overlay

    elo = Overlay(game="gates", version="0.1.0")
    if elo.connect():
        who = elo.address()                       # 0x… or None
        out = elo.sign(play_message("duel", vow_id, "ETH up 5"))
        if out.ok:
...
PROTOCOL = 1
SOCKET_ENV = "ELO_LAUNCHER_SOCKET"
...
def default_socket() -> str:
    ...
    (`elo-broker/src/transport.rs`) and with the Rust SDK's. A mismatch is the
    ...
    goes red. `$ELO_LAUNCHER_SOCKET` is set by the launcher on every native
    ...
    if os.name == "nt":
        ...
        return rf"\\.\pipe\elo-launcher-{user}"
    runtime = os.environ.get("XDG_RUNTIME_DIR")
    if runtime:
        return str(Path(runtime) / "elo" / "launcher.sock")
    return str(Path.home() / ".cache" / "elo" / "launcher" / "launcher.sock")
...
class Overlay:
    ...
    def prove(self, server: str, nonce: str) -> Result:
        ...
        backend needs no elo-specific code::
...
if __name__ == "__main__":                                  # a live smoke check
    import sys
    ov = Overlay(game="elo-overlay-demo", version="1")
    if not ov.connect():
        print(ov.why)
